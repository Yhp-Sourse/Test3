            ...
            <el-container>
                ...
                <el-header>
                    <div class="search">
                        <el-form :inline="true" class="demo-form-inline">
                            <el-form-item label="设备厂家：">
                                <el-input v-model="queryparam.FacName" placeholder="设备厂家"></el-input>
                            </el-form-item>
                           <el-form-item label="故障时间：">	   
                                <el-date-picker type="datetime" value-format="yyyy-MM-dd hh:mm:ss" :clearable=true v-model="queryparam.StartTime" placeholder="故障开始时间"></el-date-picker>
                            </el-form-item>
                           <el-form-item label="-">
                                <el-date-picker type="datetime" value-format="yyyy-MM-dd hh:mm:ss" :clearable=true v-model="queryparam.EndTime" placeholder="故障结束时间"></el-date-picker>
                            </el-form-item>
                           <el-form-item label="设备名称:">
                                <el-input v-model="queryparam.DeviceName" placeholder="设备名称"></el-input>
                            </el-form-item>
                    ...
                    <div class="tools">
                         ...
                         <el-button size="small" class=" el-button--iconButton" icon="el-icon-download"  v-has="'usr_handleExport'" style="text-overflow: initial;" @click="download();">导出</el-button>
                    </div>
                </el-header>
                    ...
                    <el-row>     
                        <el-col :span="12">
                            <div> 
                                ...
                                <el-select v-model="ruleForm.FacId" placeholder="设备厂家"  @change="change1">
                                    <el-option v-for="item in iconList" :key="item.facId" :label="item.facName" :value="item.facId"></el-option>
                                </el-select>   
                                </el-form-item>
                            </div>
                        </el-col>                                       
                        <el-col :span="12">
                            <div>
                                ...
                                <el-select v-model="ruleForm.DeviceId" placeholder="设备名称"  @change="change2">
                                    <el-option v-for="item in iconList2" :key="item.id" :label="item.deviceName" :value="item.id"></el-option>
                                </el-select>                            
                                </el-form-item>
                            </div>
                        </el-col>
                        ...
                        <el-col :span="12">
                            <div> 
                                <el-form-item label="故障现象：" prop="FailurePhenomenon">
                                    <el-input type="textarea" :rows="6" v-model="ruleForm.FailurePhenomenon" autocomplete="off"></el-input>
                                </el-form-item>
                            </div>
                        </el-col>                                        
                         <el-col :span="12">
                            <div> 
                                <el-form-item label="解决方法：" prop="Resolvent">
                                    <el-input type="textarea" :rows="6" v-model="ruleForm.Resolvent" autocomplete="off"></el-input>
                                </el-form-item>
                            </div>
                        </el-col>                      
                    </el-row>
...
export default {
    ...
    data() { 
      return { 
        ...
        rules: {
            ...
            FailureReason: [{ required: false, message: '请输入故障现象', trigger: 'blur' }],
            FailurePhenomenon: [{ required: true, message: '请输入故障现象', trigger: 'blur' }],
            Resolvent: [{ required: true, message: '请输入解决方法', trigger: 'blur' }],
        },
        queryparam:{
            FacName:'',
            StartTime:'',
        ...
        page:{   //关于页码的相关参数
            ...
            columns: [
              ...
              {prop: 'deviceName', label: '设备型号', align: 'center',isShow:true },
              {prop: 'facName', label: '设备厂家', align: 'center',isShow:true },
              {prop: 'failurePhenomenon', label: '故障现象',width:200, align: 'center',isShow:true },
              {prop: 'resolvent', label: '解决办法', width:200,align: 'center',isShow:true },
              {prop: 'show_StartTime', label: '故障开始时间', align: 'center',isShow:true },
              {prop: 'show_EndTime', label: '故障结束时间', align: 'center',isShow:true },
              {prop: 'show_FailureHour', label: '故障时长（小时）', align: 'center',isShow:true },
    ...
    watch:{
        selectFacId(oldVal,newVal){
            if(this.selectFacId){
                ...
                this.iconList2=Enumerable.from(this.iconList).where(p=>p.facId==this.selectFacId).toArray();             
            }
        }
    },
    methods:{ 
        ...
        getAllIcons(SStation){  
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/MM_EquipmentFaultData/GetDeviceBySStation?facId=0&SStation=' + SStation
            }).then(res => {
                if(res.status==200){
        ...
        change1(val){
            ...
            var Enumerable = require('linq');
            this.iconList2=Enumerable.from(this.iconList).where(p=>p.facId==val).toArray();             
        },
        change2(val){
        ...
        getList(obj){      
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/MM_EquipmentFaultData/GetUsrChangeLogList?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+'&FacName=' + self.queryparam.FacName+'&StartTime=' + self.queryparam.StartTime+'&EndTime=' + self.queryparam.EndTime+'&DeviceName=' + self.queryparam.DeviceName+'&SStations=' + self.queryparam.chooseStationIds
            }).then(res => {
                if(res.status==200){
       ...
       //依据按钮id获取信息
        getFunInfoById(id){  
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/MM_EquipmentFaultData/GetUsrChangeLogById?Id=' + id
            }).then(res => {
                if(res.status==200){
        ...
        initChartData (){
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/MM_EquipmentFaultData/GetGroupData?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+'&FacName=' + self.queryparam.FacName+'&StartTime=' + self.queryparam.StartTime+'&EndTime=' + self.queryparam.EndTime+'&DeviceName=' + self.queryparam.DeviceName+'&SStations=' + self.queryparam.chooseStationIds
            }).then(res => {
                if(res.status==200){
                    ...
                    for(var i=0;i<a.length;i++){
                        if(a[i].typeName!=null){
                            ...
                            arr1.push(
                                {value: parseInt(a[i].toal), name: a[i].typeName},
                            );
                        }
        ...
        download(obj){
            ...
            this.$http({
                ...
                url: this.api+'/api/MM_EquipmentFaultData/GetUsrListDownLoad?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+'&FacName=' + self.queryparam.FacName+'&StartTime=' + self.queryparam.StartTime+'&EndTime=' + self.queryparam.EndTime+'&DeviceName=' + self.queryparam.DeviceName+'&SStations=' + self.queryparam.chooseStationIds
            }).then(res => {
                if(res.status==200){              
    ...
    components:{
        rateTable,rateCascader,treeSStation,echartsTemplate
    },
    created(){
        //调用获取列表页的方法
    },
