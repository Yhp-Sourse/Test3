<template>
                    ...
                    <div class="search">
                        <el-form :inline="true" class="demo-form-inline">
                            ...
                            <el-form-item class="btn">
                                <el-button type="primary" icon="el-icon-search"  v-has="'operationCost_handleSearch'"  @click="getList();">查询</el-button>
                                <el-button type="primary" icon="el-icon-download"  @click="download();">导出</el-button>
                            </el-form-item>
                        </el-form>
                    </div>
                    <div class="tools">
                         <el-button size="small" class=" el-button--iconButton" icon="el-icon-plus"  v-has="'operationCost_handleAdd'" style="text-overflow: initial;" @click="handleAdd">添加</el-button> 
                         <el-button size="small" class=" el-button--iconButton" icon="el-icon-delete"  v-has="'operationCost_handleMultiplDel'"  style="text-overflow: initial;" @click="handleMultiplDel">删除</el-button>
                    </div>
                </el-header>
    ...
    <el-table
      ...
      <el-table-column
        ...
        width="180">
         <template slot-scope="scope">         
            <el-input  v-model="scope.row.unitName" placeholder="" :disabled="true"></el-input>               
         </template>
      </el-table-column>

      <el-table-column
        prop="itemCost"
        label="单站费用/月(元)" width="180">
         <template slot-scope="scope">         
               <el-input  v-model="scope.row.itemCost" placeholder="请输入价格"  ></el-input>          
         </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        label="备注">
         <template slot-scope="scope">          
           <el-input  v-model="scope.row.itemRemark" placeholder="限500字以内" type="textarea" :rows="2"></el-input>
         </template>

      </el-table-column>
    </el-table>
                <el-footer>
                   <el-button size="small" class=" el-button--iconButton"  style="text-overflow: initial;" @click="submitAddConfig()">保存</el-button> 
                    <el-button size="small" class=" el-button--iconButton"  style="text-overflow: initial;" @click="closeDig()">取消</el-button> 
                </el-footer>
    </el-form>                
            </el-dialog>
...
export default {
    ...
    data() {  
      return {           
        ...
        rules: {
            itemCost: [{ required: true, message: '请输入运维费用', trigger: 'blur' }],                 
        },
        queryparam:{
        ...
        page:{   //关于页码的相关参数
            ...
            columns: [
              ...
              {prop: 'show_Status', label: '状态', width:180,align: 'center',isShow:true },
              ...
              {prop: 'show_CreatedTime', label: '添加时间', align: 'center',isShow:true }
            ],// 需要展示的列
            operates: {   //操作栏
              width:200,      
              fixed: 'right',
    ...
    methods:{    
        ...
        submitAddConfig(){
           ...
           this.$http({
                method: 'GET',
                url: self.api+'/api/Yw_CostConfig/YwCostConfigSubmit?jsonStr=' + jsonStr + '&year=' + this.choseYear
            }).then(res => {
                if(res.status==200){                
        ...
        handleClose(done){
           this.$confirm('确认关闭?').then(_=>{
                ...
                this.resetForm('ruleForm'); this.choseYear='';})
        ...
        handlemultipl(ids){
            ...
            this.$http({
                method: 'GET',
                url: self.api+'/api/Yw_CostConfig/DeleteOperationCostConfig?conFigId='+ids
            }).then(res => {
                if(res.status==200){
                    self.getList();
                    self.$message({
        ...
        getList(obj){      
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/Yw_CostConfig/GetCostConfigPageList?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+'&status=' + self.Status
            }).then(res => {
                if(res.status==200){
        ...
        getYunCostById(id,year){          
            ...
            this.$http({
                method: 'GET',
                url: self.api+'/api/Yw_CostConfig/GetYwCostById?id=' + id +'&year=' +year
            }).then(res => {
                if(res.status==200){
                    self.ruleForm.unitList=res.data.data;
        ...
        download(obj){
            ...
            this.$http({
                ...
                url: this.api+'/api/Yw_CostConfig/GetCostConfigDownLoad?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+'&status=' + self.Status
            }).then(res => {
                if(res.status==200){              
