                            ...
                            <div class="search">
                                <el-form :inline="true" class="demo-form-inline" size="mini">                                  
                                    ...
                                    <el-form-item class="btn">
                                        <el-button size="small" icon="el-icon-search" v-has="'logManagement_handleSearch'"  type="primary" @click="getList();">查询</el-button>
                                        <el-button type="primary" icon="el-icon-download"  @click="downloadoperate();">导出</el-button>
                                    </el-form-item>
                                </el-form>
                            </div>                        
                        </el-header>
                        ...
                        <el-header>
                            <div class="search">
                                <el-form :inline="true" class="demo-form-inline" size="mini">                            
                                    ...
                                    <el-form-item class="btn">
                                        <el-button size="small" icon="el-icon-search"   v-has="'logManagement_handleSearch'"   type="primary" @click="getList_Repair();">查询</el-button>
                                         <el-button type="primary" icon="el-icon-download"  @click="downloadlogin();">导出</el-button>
                                    </el-form-item>
                                </el-form>
                            </div>                        
                        </el-header>
    ...
    data(){
        return {                  
            ...
            columns: [
                ...
                { prop: 'show_ActionDate', label: '操作时间',  align: 'center' ,isShow:true },
            ],// 需要展示的列
            operates: {   //操作栏
                width:280,      
                fixed: 'right',
    ...
    methods:{
       ...
       getNowTime() {
         ...
         this.$set(this.queryparam, "EndDate", defaultDate);
          ...
          //登录日志
         this.$set(this.queryparam_Repair, "StartDate", beforedefaultDate);     
         this.$set(this.queryparam_Repair, "EndDate", defaultDate);
        ...
        getList(obj){
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/Yw_LogManagement/GetOperateLog?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+ '&StartActionDate=' + self.queryparam.StartDate + '&KeyWord='+self.queryparam.KeyWord+'&EndActionDate=' + self.queryparam.EndDate
            }).then(res => {
                if(res.status==200){
        ...
        getList_Repair(obj){
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/Yw_LogManagement/GetLoginLog?pageSize=' + self.page_Repair.pageSize + '&pageIndex=' + self.page_Repair.pageNo+'&StartLoginDate='+ self.queryparam_Repair.StartDate+'&EndLoginDate='+self.queryparam_Repair.EndDate
            }).then(res => {
                if(res.status==200){
        ...
        downloadoperate(){
            ...
            this.$http({
                ...
                url: this.api+'/api/Yw_LogManagement/GetOperateLogDownLoad?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+ '&StartActionDate=' + self.queryparam.StartDate + '&KeyWord='+self.queryparam.KeyWord+'&EndActionDate=' + self.queryparam.EndDate
            }).then(res => {
                if(res.status==200){              
    ...
    document.body.removeChild(elink);
        ...
        downloadlogin(){
            ...
            this.$http({
                method: 'GET',
                responseType: 'blob', // 表明返回服务器返回的数据类型
                url: this.api+'/api/Yw_LogManagement/GetLoginLogDownLoad?pageSize=' + self.page_Repair.pageSize + '&pageIndex=' + self.page_Repair.pageNo+'&StartLoginDate='+ self.queryparam_Repair.StartDate+'&EndLoginDate='+self.queryparam_Repair.EndDate
            }).then(res => {
                if(res.status==200){              
    let blob = new Blob([res.data], {type: 'application/vnd.ms-excel' })
    let  downTime= self.downLoadDate();
