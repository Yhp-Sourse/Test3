    ...
    <div id="v_deviceOnlineInfo">
                    ...
                    <div class="search">
                        <el-form :inline="true" class="demo-form-inline">
                            <el-form-item label="角色名称">
                                <el-input v-model="queryparam.RoleName" placeholder="角色名称"></el-input>
                            </el-form-item>
                          
                            <el-form-item class="btn">
                                <el-button type="primary" icon="el-icon-search" v-has="'role_handleSearch'" @click="getList();">查询</el-button>
                                <el-button type="primary" icon="el-icon-download"  @click="download();">导出</el-button>
                            </el-form-item>
                        </el-form>
                    </div>
                    <div class="tools">
                         <el-button size="small" class=" el-button--iconButton" icon="el-icon-plus"  v-has="'role_handleAdd'"  style="text-overflow: initial;" @click="handleAdd">添加</el-button> 
                         <el-button size="small" class=" el-button--iconButton" icon="el-icon-delete"  v-has="'role_handleMultiplDel'" style="text-overflow: initial;" @click="handleMultiplDel">删除</el-button>
                    </div>
                </el-header>
        ...
        <div>
                        ...
                        <el-col :span="12">
                            <div>
                                <el-form-item label="角色名称：" prop="role_name">
                                    <el-input v-model="ruleForm.role_name" autocomplete="off"></el-input>
                                </el-form-item>
                            </div>
                        </el-col>
                        <el-col :span="12">
                            <div> 
                                <el-form-item label="角色编码：" prop="role_code">
                                    <el-input v-model="ruleForm.role_code" autocomplete="off" ></el-input>
                                </el-form-item>
                            </div>
                        </el-col>
                    </el-row>
                        ...
                        <el-col :span="12">
                            <div>
                                <el-form-item label="角色排序：" prop="role_order">
                                    <el-input v-model="ruleForm.role_order" autocomplete="off"></el-input>
                                </el-form-item>
                            </div>
                        </el-col>
                        <el-col :span="6">
                        ...
                        <el-col :span="24">
                            <div>
                                <el-form-item label="角色描述：" prop="role_beizhu">
                                    <el-input v-model="ruleForm.role_beizhu" autocomplete="off" type="textarea" :rows="3"></el-input>
                                </el-form-item>
                            </div>
                        </el-col>
            ...
            <el-dialog
                ...
                <el-main>        
                    <treeSStation :CheckData='CheckData' :IsInput='false' @checkedNodes="getFunctions"></treeSStation>                   
                </el-main>              
            </el-dialog>
        </div>
    </div>
...
export default {
    ...
    data() {   
      return {             
        ...
        rules: {
            role_order: [{ required: true, message: '请填写用户排序', trigger: 'blur' }],
            role_code: [{ required: true, message: '请填写角色编码', trigger: 'blur' }],
            role_name: [{ required: true, message: '请填写角色名称', trigger: 'blur' }],
            role_status: [{ required: true, message: '请选择是否启用', trigger: 'blur' }],
        },
        queryparam:{
            RoleName:'',
        },
        page:{   //关于页码的相关参数
            ...
            columns: [
              ...
              {prop: 'role_code', label: '角色编码', align: 'center',isShow:false },
              {prop: 'role_name', label: '角色名称', width:180,align: 'center',isShow:true },
               {prop: 'role_status', label: '是否启用', align: 'center',isShow:true },
              {prop: 'role_beizhu', label: '角色描述', align: 'center',isShow:true }, 
              {prop: 'role_order', label: '角色排序',align: 'center',isShow:true },
    ...
    methods:{
       ...
       RefrshSessionPerssion(){
        ...
        this.$http({
            ...
            if(res.status==200){        
              sessionStorage.setItem("functionData", JSON.stringify(Enumerable.from(res.data.data).where(o=>o.menu_type==2).toArray()));
            }
          }).catch(error => {
            console.log(error);
          });
        ...
        //获取权限数据
        handlPermission(index, row) {  //获取分配站点信息      
            var self=this;
        ...
        getList(obj){      
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/Yw_Roles/GetRolesPageList?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+'&roleName=' + self.queryparam.RoleName
            }).then(res => {
                if(res.status==200){
        ...
        download(obj){
            ...
            this.$http({
                ...
                url: this.api+'/api/Yw_Roles/GetRolesDownLoad?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+'&roleName=' + self.queryparam.RoleName
            }).then(res => {
                if(res.status==200){              
