                    ...
                    <div class="search">
                        <el-form :inline="true" class="demo-form-inline" >
                            ...
                            <el-form-item class="btn">
                                <el-button type="primary" v-has="'JcTotal_handleSearch'"  icon="el-icon-search" @click="getList();">查询</el-button>
                                <el-button type="primary" icon="el-icon-download"  v-has="'JcTotal_handleExport'"  @click="download();">导出</el-button>
                            </el-form-item>
                        </el-form>
                    </div>
                    <div class="tools">
...
export default {
    data() {
      return {
        ...
        //   {prop: 'unitName',label: '运维单位',align: 'center',isShow:true,formatter(){return '武汉天虹'}},
          ...
          {prop: 'score',label: '分数',align: 'center',isShow:true,formatter(row, column, cellValue){
              return "<a style='color:#01AAED;'>"+row.score+"</a>";
          }},
          {prop: 'markedBy',label: '打分人',align: 'center',isShow:true},
          {prop: 'markedTime',label: '时间',align: 'center',isShow:true,width:200,formatter(row, column, cellValue){
              if(row.markedTime){
    ...
    methods:{
        ...
        handleView(markId,sStation,markMonth) { 
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/Jx/GetScoreSY?markId=' + markId + '&sStation=' + sStation+ '&markMonth=' + markMonth
            }).then(res => {
                if(res.status==200){
                    if(res.data.data.taskNo==''){
                        ...
                        let obj = { taskNo: res.data.data.taskNo,type: 'view', fromTab:'' };    
                        self.$emit('jump',{param:'第四方检查打分',path:'/index/ywFourthInspectionTaskShow?obj='+ JSON.stringify(obj),isjump:true});
                    }
        ...
        getList(obj){
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/Jx/GetJcList?pagesize=' + self.page.pageSize + '&pageindex=' + self.page.pageNo+ '&UnitId=' + self.queryparam.UnitId + '&MarkMonth=' + self.queryparam.MarkMonth  +'&SStations=' + self.queryparam.chooseStationIds
            }).then(res => {
                if(res.status==200){
        ...
        download(obj){
            ...
            this.$http({
                ...
                url: this.api+'/api/Jx/GetJcListDownLoad?pagesize=' + self.page.pageSize + '&pageindex=' + self.page.pageNo+ '&UnitId=' + self.queryparam.UnitId + '&MarkMonth=' + self.queryparam.MarkMonth  +'&SStations=' + self.queryparam.chooseStationIds
            }).then(res => {
                if(res.status==200){              
