                    ...
                    <div class="search">
                        <el-form :inline="true" class="demo-form-inline">  
                          <el-form-item label="任务编号：">
                                <el-input v-model="queryparam.TaskNo" placeholder="请输入编号"></el-input>
                          </el-form-item>                      
                          <!--<el-form-item label="检查类型：">
                                <el-select v-model='queryparam.TaskType'   placeholder='请选择任务类型' disabled style="width: 100%;">
                          ...
                          <el-form-item label="检查时间：">
                            ...
                            <el-form-item class="btn">
                                <el-button type="primary" icon="el-icon-search"  v-has="'ywFourthInspectionTask_handleSearch'"   @click="getList();">查询</el-button>
                                <el-button type="primary" icon="el-icon-download"  v-has="'ywFourthInspectionTask_handleExport'"    @click="download();">导出</el-button>
                            </el-form-item>
                        </el-form>
                    </div>
                    <div class="tools">
                         <el-button size="small" class=" el-button--iconButton" icon="el-icon-plus"  v-has="'ywFourthInspectionTask_handleAdd'"  style="text-overflow: initial;" @click="handleAdd">添加</el-button> 
                         <el-button size="small" class=" el-button--iconButton" icon="el-icon-s-promotion" v-has="'ywFourthInspectionTask_handleArrangement'"  style="text-overflow: initial;" @click="handleArrangement">领取</el-button>
                         <el-button size="small" class=" el-button--iconButton" icon="el-icon-delete" v-has="'ywFourthInspectionTask_handleMultiplDel'"  style="text-overflow: initial;" @click="handleMultiplDel">删除</el-button>
                         <el-button size="small" class=" el-button--iconButton" icon="el-icon-right"  v-has="'ywFourthInspectionTask_handleRecall'"  style="text-overflow: initial;" @click="handleBatchCancel">撤回</el-button>
                    </div>
                </el-header>
...
export default {
      ...
      return {         
        ...
        page:{   //关于页码的相关参数
            ...
            columns: [
              ...
              {prop: 'show_TaskType', label: '任务类型', width:120,align: 'center',isShow:true },
              {prop: 'categoryName', label: '任务类别', align: 'center',isShow:true },
              {prop: 'show_IsPlan', label: '计划', width:180, align: 'center',isShow:true },
              ...
              {prop: 'show_GenModifyProblem', label: '第四方是否确认', align: 'center',isShow:true },
              {prop: 'unitName', label: '运维单位', align: 'center',isShow:true },
              {prop: 'checkCompany', label: '检查单位', align: 'center',isShow:true },
              {prop: 'dealerName', label: '检查人员', align: 'center',isShow:true },
              {prop: 'nextAuditerId', label: '上级人员Id', align: 'center',isShow:false },
              {prop: 'reviewerName', label: '确认人', align: 'center',isShow:true },
              {prop: 'show_DealTime', label: '检查时间', align: 'center',isShow:false },
              {prop: 'show_MaintainceWay', label: '检查方式', align: 'center',isShow:true },
              {prop: 'show_CreatedTime', label: '创建时间', align: 'center',isShow:true },
              {prop: 'createdBy', label: '创建人', align: 'center',isShow:true },
              {prop: 'remark', label: '备注', align: 'center',isShow:true },
              {prop: 'show_PlanDealTime', label: '计划检查时间', align: 'center',isShow:true },
              {prop: 'planDealTime', label: '计划检查时间', align: 'center',isShow:false },
              {prop: 'dealTime', label: '检查时间', align: 'center',isShow:false },
              {prop: 'taskType', label: '任务类型', align: 'center',isShow:false },           
              {prop: 'maintainceWay', label: '处理方式', align: 'center',isShow:false },
              {prop: 'createdTime', label: '创建时间', align: 'center',isShow:false },
              {prop: 'unitId', label: '运维单位Id', align: 'center',isShow:false },           
              {prop: 'dealerId', label: '检查人Id', align: 'center',isShow:false },
              {prop: 'ssGenModifyProblem', label: '创建时间', align: 'center',isShow:false }
            ],// 需要展示的列
            operates: {   //操作栏
              width:200,      
              fixed: 'right',
              list: [
                    {
                      ...
                      hasbutton:'ywFourthInspectionTask_handleEdit',
                      disabled: false,   //是否禁用按钮 默认是danger的禁用样式
                      method: (index, row) => {
                        var self = this;
                        if(row.status!=1){
                            self.$message({
                            message: "只有待领取的任务才能编辑!",
                      ...
                      hasbutton:'ywFourthInspectionTask_handleOperate',
                      disabled: false,   //是否禁用按钮 默认是danger的禁用样式
                      method: (index, row) => {
                        var self = this;
                        if(row.status==1){
                    ...
                    , {
                      ...
                      disabled: false,   //是否禁用按钮 默认是danger的禁用样式
                      method: (index, row) => {
                           this.handleViewReason(index, row)                                                            
                      }
                    }
                  ]
                 }, // 列操作按钮
    ...
    methods:{      
        ...
        //编辑，提交
        handleOperate (index, row) {  //操作栏编辑按钮
            var self=this;
            let obj = { taskNo: row.taskNo,type: '',fromTab:'',taskStatus:row.status };              
            this.$emit('jump',{param:'第四方检查任务窗口',path:'/index/ywFourthInspectionTaskShow?obj='+JSON.stringify(obj),isjump:true});  
            //关闭当前tab
            this.$emit("closeCurrentPage", "第四方检查任务"); //调用closeCurrentPage方法注意；两点1：第二个参数必须是string，2：长度必须大于等于4       
        },
        ...
        handleView(index, row){
            debugger;
            var self=this;
            let obj = { taskNo: row.taskNo,type: 'view', fromTab:'',taskStatus:row.status };              
            this.$emit('jump',{param:'第四方检查任务窗口',path:'/index/ywFourthInspectionTaskShow?obj='+JSON.stringify(obj),isjump:true});  
            //关闭当前tab
            this.$emit("closeCurrentPage", "第四方检查任务"); //调用closeCurrentPage方法注意；两点1：第二个参数必须是string，2：长度必须大于等于4 
        },
        ...
        //查看审核不通过原因
        handleViewReason(index, row){
            var self = this;
            this.$http({
                method: 'GET',
                url: self.api+'/api/Common/getReason?busineessId='+row.taskNo
            }).then(res => {
                self.$message({
        ...
        handleArrangement(){
            ...
            this.handleSelection.forEach((item,index) => {
                //只有删除待下发，待领取的数据
                if(item.status!=1){
                    tips += "选中第" + (index+1) + "行不能下发,存在状态不为待领取的数据，请核对!";
                }
                else
                {
                    ids += item.taskNo + ','
            ...
            if(tips!=''){
                ...
                return;
                 ...
                 this.$confirm('确认领取该任务？').then(function () {
                   self.ArrangeTask(ids);
                 }).catch(function () {               
                 });      
            } 
        },
       ...
       ArrangeTask(ids){
            ...
            this.$http({
                method: 'GET',
                url: self.api+'/api/FourthInspectionTask/ArrangeMentFourthTask?taskNo='+ids
            }).then(res => {
                if(res.status==200){
                    self.getList();
                    self.$message({
        ...
        handleMultiplDel() {
            ...
            this.handleSelection.forEach((item,index) => {
                ...
                {
                    tips += "选中第" + (index+1) + "行不能删除,只有待下发，待领取的数据能删除!";
                }
            });
            if(tips!=''){
                self.$message({
        ...
        handlemultipl(ids){
            ...
            this.$http({
                method: 'GET',
                url: self.api+'/api/InspectionPlan/DeleteTask?taskNo='+ids
            }).then(res => {
                if(res.status==200){
                    self.getList();
                    self.$message({
        ...
        handleBatchCancel(){
            ...
            this.handleSelection.forEach((item,index) => {
                //只能撤回待领取后的状态 ---   待处理、处理中、待审核  -----已完成无法撤回
                if(item.status>=2&&item.status<8){
                    ids += item.taskNo + ','
                }
                else
                {
                    tips += "选中第" + (index+1) + "行不能撤回,领取后,但未完成的任务才能撤回!";
                }
            });
            if(tips!=''){
                self.$message({
        ...
        handleRevert(ids){
           var self = this;
            this.$http({
                method: 'GET',
                url: self.api+'/api/InspectionPlan/BatchUpdateTask?taskNo='+ids
            }).then(res => {
                if(res.status==200){
                    self.getList();
                    self.$message({
        ...
        getList(obj){
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/InspectionPlan/GetInspectionTaskByPage?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+'&SStation='+self.queryparam.chooseStationIds+ ' &StartDate=' + self.queryparam.StartDate + '&EndDate=' + self.queryparam.EndDate +'&TaskType=' + self.queryparam.TaskType +' &TaskNo=' + self.queryparam.TaskNo +'&Status=' + self.queryparam.Status
            }).then(res => {
                if(res.status==200){
        ...
        download(obj){
            ...
            this.$http({
                ...
                url: this.api+'/api/InspectionPlan/GetInspectionTaskDownLoad?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+'&SStation='+self.queryparam.chooseStationIds+ ' &StartDate=' + self.queryparam.StartDate + '&EndDate=' + self.queryparam.EndDate +'&TaskType=' + self.queryparam.TaskType +' &TaskNo=' + self.queryparam.TaskNo +'&Status=' + self.queryparam.Status
            }).then(res => {
                if(res.status==200){              
    let blob = new Blob([res.data], {type: 'application/vnd.ms-excel' })
    let  downTime= self.downLoadDate();
    const fileName = downTime +'-第四方检查任务.xls';
