    ...
    <div id="v_ywTask">
        <el-container style="height: calc(100vh - 102px); border: 1px solid #eee">
            <el-aside width="250px">
                <treeSStation :IsCheckBox='true' @checkedNodes="getSearchStations" :CheckData='CheckData'></treeSStation>
            </el-aside>           
            <el-container>
                <el-header>
                    <div class="search">
                        <el-form :inline="true" class="demo-form-inline" size="mini">
                            <el-form-item label="任务编号:">
                                <el-input v-model="queryparam.TaskNo" placeholder="任务编号" clearable></el-input>
                            </el-form-item>
                            <el-form-item label="任务类型：">
                                <rate-select
                                    v-model="rateRWLXSelect.model"
                            ...
                            <el-form-item label="故障类型：">
                                <el-select style="width:100%;" v-model="queryparam.YJType" clearable placeholder="全部">
                                    <el-option
                            ...
                            <el-form-item class="btn">
                                <el-button type="primary" v-has="'ywTask_handleSearch'"  icon="el-icon-search"   @click="getTaskList();">查询</el-button>
                                <el-button type="primary" v-has="'ywTask_handleExport'" icon="el-icon-download"   @click="download();">导出</el-button>
                                ...
                                <el-button type="primary" v-has="'ywTask_handleProYwTask'" icon="el-icon-download"   @click="proTask();">生成任务</el-button>
                                <!--<el-button type="primary" v-has="'ywTask_handleSearch'" icon="el-icon-download"   @click="toDoTask();">数据处理</el-button>-->
                            </el-form-item>
                        </el-form>                       
                    </div>
                    <div class="tools">
                        <el-button size="small" class=" el-button--iconButton" v-has="'ywTask_handleAdd'" icon="el-icon-plus"  @click="handleAdd">添加应急任务</el-button>
                        ...
                        <el-button size="small" class=" el-button--iconButton" v-has="'ywTask_handleMultiplDel'" icon="el-icon-delete"  @click="handleMultiplDel">删除</el-button>
                        <el-button size="small" class=" el-button--iconButton" icon="el-icon-back"  v-has="'ywTask_handleRecall'"  style="text-overflow: initial;" @click="handleBatchCancel">撤回</el-button>
                        <el-button size="small" class=" el-button--iconButton" icon="el-icon-s-opportunity"  v-has="'ywTask_handleSD'"  style="text-overflow: initial;" @click="handleBatchSD">领取任务</el-button>
                        <el-button size="small" class=" el-button--iconButton" icon="el-icon-unlock"  v-has="'ywTask_handleUnlock'"  style="text-overflow: initial;" @click="handleBatchJS">解锁任务</el-button>
                        <el-button size="small" class=" el-button--iconButton" icon="el-icon-lock"  v-has="'ywTask_handlelock'"  style="text-overflow: initial;" @click="handleBatchSuoD">锁定任务</el-button>
                    </div>
                </el-header>
                        ...
                        <el-col :span="12">
                            <div>
                                <el-form-item label="内容：" prop="Remark">
                                    <el-input  type="textarea" :rows="8" v-model="form.YSRemark" autocomplete="off"></el-input>
                                </el-form-item>
                            </div>
                        </el-col>
                        <el-col :span="12">
                            <div>
                                <el-form-item v-show="true" label="附件：" prop="TaskNo">
                                    <rateUpload :limit='10' :Ismultiple='true' :BusinessId='form.TaskNo' :BusinessType='22' @uploadSuccess='uploadSuccess'></rateUpload>
                                </el-form-item>
                            </div>
                        </el-col>
                    </el-row>
...
export default {
    ...
    data() {       
      ...
      return {  
        ...
        columns: [
          {prop: 'planDealTime',label: '时间周期',align: 'center',isShow:true,width:200,formatter:this.timePeriod},
          {prop: 'show_Status',label: '任务状态',align: 'center',isShow:true},
          {prop:'city',label: '城市',align: 'center',isShow:true},
          {prop:'sStationName',label:'站点名称',align:'center',isShow:true,width:150},
          {prop:'show_TaskType',label:'任务类型',align:'center',isShow:true},
          {prop:'show_TaskCategoryName',label:'任务类别',align:'center',isShow:true},
          {prop: 'taskNo',label: '任务编号',align: 'center',isShow:true,width:200},//formatter重构//return `返回参数`
          {prop: 'title',label: '标题',align: 'center',isShow:true,width:150},
          {prop: 'planNo',label: '计划编号',align: 'center',isShow:true,width:200},        
          {prop: 'show_Urgency',label: '紧急程度',align: 'center',isShow:true},
          {prop: 'unitName',label: '运维单位',align: 'center',isShow:true},
          {prop: 'dealerName',label: '运维人员',align: 'center',isShow:true},
          {prop: 'nextAuditerName',label: '审批人',align: 'center',isShow:true},
          {prop: 'sendingTime',label: '任务领取时间',align: 'center',isShow:true,width:200,formatter:this.formatterTime},
          {prop: 'planDealTime',label: '任务截止日期',align: 'center',isShow:true,width:200,formatter:this.formatterTime},        
          {prop: 'dealTime',label: '处理时间',align: 'center',isShow:false,width:200,formatter:this.formatterTime},
          {prop: 'show_MaintainceWay',label: '处理方式',align: 'center',isShow:true},
          {prop: 'createdTime',label: '创建时间',align: 'center',isShow:true,width:200,formatter:this.formatterTime},
          {prop: 'show_YJType',label: '应急类型',align: 'center',isShow:true,width:200},
          {prop: 'remark',label: '备注',align: 'center',isShow:true},
          {prop: 'ysFileName',label: '延时完成附件',align: 'center',isShow:true,width:200,
            formatter: (row, column, cellValue) => {
                if(row.ysFileName){return '<a style="color:blue;">'+row.ysFileName+'</a>';}
                else{return '';}
            }
          },
          ...
          list: [
                {
                  ...
                  method: (index, row) => {
                    //周，月，两月，季，半年，年进行控制
                    if(row.isLock==0 && row.taskCategoryId==1 && self.timePeriodDetail(1, row.planDealTime)!=self.timePeriodDetail(1, new Date())){ //周
                        this.$message({
                            type:"warning",
                            dangerouslyUseHTMLString: true,
                            message: "不能处理，当前时间为【"+self.timePeriodDetail(1, new Date())+"】！"
                        });
                        return false;
                    }
                    if(row.isLock==0 && row.taskCategoryId==2 && self.timePeriodDetail(2, row.planDealTime)!=self.timePeriodDetail(2, new Date())){  //月
                        this.$message({
                            type:"warning",
                            dangerouslyUseHTMLString: true,
                    ...
                    if(row.isLock==0 && row.taskCategoryId==3 && self.timePeriodDetail(3, row.planDealTime)!=self.timePeriodDetail(3, new Date())){  //季
                        this.$message({
                            type:"warning",
                            dangerouslyUseHTMLString: true,
                            message: "不能处理，当前时间为【"+self.timePeriodDetail(3, new Date())+"】！"
                        });
                        return false;
                    }
                    if(row.isLock==0 && row.taskCategoryId==4 && self.timePeriodDetail(4, row.planDealTime)!=self.timePeriodDetail(4, new Date())){  //半年
                        this.$message({
                            type:"warning",
                            dangerouslyUseHTMLString: true,
                            message: "不能处理，当前时间为【"+self.timePeriodDetail(4, new Date())+"】！"
                        });
                        return false;
                    }
                    if(row.isLock==0 && row.taskCategoryId==5 && self.timePeriodDetail(5, row.planDealTime)!=self.timePeriodDetail(5, new Date())){  //年
                        this.$message({
                            type:"warning",
                            dangerouslyUseHTMLString: true,
                            message: "不能处理，当前时间为【"+self.timePeriodDetail(5, new Date())+"】！"
                        });
                        return false;
                    }
                    if(row.isLock==0 && row.taskCategoryId==6 && self.timePeriodDetail(6, row.planDealTime)!=self.timePeriodDetail(6, new Date())){  //两月
                        this.$message({
                            type:"warning",
                            dangerouslyUseHTMLString: true,
                            message: "不能处理，当前时间为【"+self.timePeriodDetail(6, new Date())+"】！"
                        });
                        return false;
                    }
                ...
                },
                {
                  ...
                  method: (index, row) => {
	                ...
	                var CurrentDay2=new Date(new Date().setDate(new Date().getDate()-3));
                    if(row.isLock==0 && row.show_TaskCategoryName=="周巡检" && new Date(row.planDealTime)<CurrentDay2){
    ...
    methods:{
      ...
      handleRowdblclick(row, column, event){
            ...
            if(row.row.status!=6)
            {
                this.$http({
                  method: 'GET',
                  url: self.api+'/api/Yw_Rpt/YwFormDetail?taskNo=' + row.row.taskNo +'&monthTime='+row.row.planDealTime +'&taskCategoryId=' + row.row.taskCategoryId
                  }).then(res => {
                  if(res.status==200){                   
                    self.$notify({
                         title: '消息',
      ...
      timePeriod(row,column,cellValue){
            ...
            if(row.planDealTime==null){
                ...
                return  this.timePeriodDetail(row.taskCategoryId, row.planDealTime,row.isLock);
            }
      },
      ...
      timePeriodDetail(timeType, val,isLock){
            ...
            //当周
            var currentWeek=currentDate.getFullYear() + "年第" + self.getWeekOfYear(currentDate) + "周";// 当前周日期是第几年第几周
            //当月
            var currentMonthAndTwoMonth=currentDate.getFullYear() + "年第" +currentMonth + "月"; //当前month和TwoMonth
            //当季度
            var currentQuarter = "";
            if (currentMonth <= 3) {
            ...
            var currentSeasonMonth= currentDate.getFullYear() + "年第" + currentQuarter;
            //半年
            var currentHalfYear='';
            if (currentMonth / 6 <= 1) {
                currentHalfYear = currentDate.getFullYear() + "年" + "上半年"+currentMonth+'月';
            }
            else if (currentMonth / 6 > 1) {
                currentHalfYear = currentDate.getFullYear() + "年" + "下半年"+currentMonth+'月';
            }
            //年
            ...
            switch(timeType){
               ...
               case 4:  //半年
                  var Month = today.getMonth()+1;
                  if (Month / 6 <= 1) {
                    inspectionDesc = today.getFullYear() + "年" + "上半年"+Month+'月';
                  }
                  else if (Month / 6 > 1) {
                    inspectionDesc = today.getFullYear() + "年" + "下半年"+Month+'月';
                  }
                taskHalfYear=true;
                 break;
               case 5:
            ...
            if(taskW){//是否需要锁定
                if(inspectionDesc!=currentWeek){ //判断是否为本周
                    if(isLock==1){//不是本周
            ...
            if(taskM){//是否需要锁定
                if(inspectionDesc!=currentMonthAndTwoMonth){ //判断是否为本月
                    if(isLock==1){//不是本月的  
                        return  `<i class="unlock" style="vertical-align: super;font-size: 30px;"><svg t="1614130652256" class="icon" viewBox="0 0 1448 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3130" width="32" height="32"><path d="M868.057993 437.736921H419.565235c-32.212778 0-58.320892 26.105681-58.320892 58.322108v425.769637c0 32.27481 26.108114 58.382924 58.320892 58.382925h606.019618c32.21886 0 58.266158-26.108114 58.266158-58.382925v-425.769637c-0.004865-32.21886-26.103249-58.322109-58.326973-58.322108H868.057993z m-112.727696 282.304548L755.329081 813.013843c0 4.152485-3.437295 7.655461-7.655461 7.655461h-50.133903c-4.270468 0-7.711411-3.502975-7.711412-7.655461v-92.972374c-23.553861-12.045127-39.868239-36.193764-39.868239-64.49732 0-40.045821 32.513207-72.618627 72.563893-72.618627 40.10542 0 72.623492 32.572806 72.623493 72.618627 0.053518 28.303555-16.259644 52.450976-39.817155 64.49732z" p-id="3131" fill="#1afa29"></path><path d="M932.254884 286.08241c0-80.218138 65.323194-145.541332 145.541332-145.541332 80.216921 0 145.424566 65.323194 145.424566 145.541332V437.738137h90.601784V282.229137c0-130.235275-105.850674-236.1419-236.083517-236.1419-130.234059 0-236.201499 105.906624-236.201499 236.1419V437.738137h90.717334V286.08241z" p-id="3132" fill="#1afa29"></path></svg></i>${inspectionDesc}`;
                    }else{//未手动解锁   
            ...
            if(taskSeason){//是否需要锁定
                if(inspectionDesc!=currentSeasonMonth){ //判断是否为季度本月
                    if(isLock==1){//不是本月的
                        return  `<i class="unlock" style="vertical-align: super;font-size: 30px;"><svg t="1614130652256" class="icon" viewBox="0 0 1448 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3130" width="32" height="32"><path d="M868.057993 437.736921H419.565235c-32.212778 0-58.320892 26.105681-58.320892 58.322108v425.769637c0 32.27481 26.108114 58.382924 58.320892 58.382925h606.019618c32.21886 0 58.266158-26.108114 58.266158-58.382925v-425.769637c-0.004865-32.21886-26.103249-58.322109-58.326973-58.322108H868.057993z m-112.727696 282.304548L755.329081 813.013843c0 4.152485-3.437295 7.655461-7.655461 7.655461h-50.133903c-4.270468 0-7.711411-3.502975-7.711412-7.655461v-92.972374c-23.553861-12.045127-39.868239-36.193764-39.868239-64.49732 0-40.045821 32.513207-72.618627 72.563893-72.618627 40.10542 0 72.623492 32.572806 72.623493 72.618627 0.053518 28.303555-16.259644 52.450976-39.817155 64.49732z" p-id="3131" fill="#1afa29"></path><path d="M932.254884 286.08241c0-80.218138 65.323194-145.541332 145.541332-145.541332 80.216921 0 145.424566 65.323194 145.424566 145.541332V437.738137h90.601784V282.229137c0-130.235275-105.850674-236.1419-236.083517-236.1419-130.234059 0-236.201499 105.906624-236.201499 236.1419V437.738137h90.717334V286.08241z" p-id="3132" fill="#1afa29"></path></svg></i>${inspectionDesc}`;
                    }else{//未手动解锁
                        return  `<i class="lock" style="vertical-align: super;font-size: 30px;"><svg t="1614132616909" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="6593" width="32" height="32"><path d="M721.92 493.24032l0-74.26048c0 0-0.02048-173.21984-179.2-173.21984-179.17952 0-179.2 173.21984-179.2 173.21984l0 74.26048L286.72 493.24032 286.72 839.68l512 0L798.72 493.24032 721.92 493.24032zM568.32 684.3392l0 81.1008-51.2 0 0-81.1008C501.88288 675.75808 491.52 659.968 491.52 641.69984c0-27.3408 22.9376-49.50016 51.2-49.50016 28.2624 0 51.2 22.13888 51.2 49.50016C593.92 659.968 583.55712 675.75808 568.32 684.3392zM670.72 493.21984l-256 0 0-73.91232c0.28672-20.72576 7.80288-124.04736 128-124.04736 120.17664 0 127.71328 103.3216 128 123.71968L670.72 493.21984z" p-id="6594" fill="#d81e06"></path></svg></i>${inspectionDesc}`;
            ...
            if(taskHalfYear){//是否需要锁定
                if(inspectionDesc!=currentHalfYear){ //判断是否为半年月
                    if(isLock==1){//不是本月的
                        return  `<i class="unlock" style="vertical-align: super;font-size: 30px;"><svg t="1614130652256" class="icon" viewBox="0 0 1448 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3130" width="32" height="32"><path d="M868.057993 437.736921H419.565235c-32.212778 0-58.320892 26.105681-58.320892 58.322108v425.769637c0 32.27481 26.108114 58.382924 58.320892 58.382925h606.019618c32.21886 0 58.266158-26.108114 58.266158-58.382925v-425.769637c-0.004865-32.21886-26.103249-58.322109-58.326973-58.322108H868.057993z m-112.727696 282.304548L755.329081 813.013843c0 4.152485-3.437295 7.655461-7.655461 7.655461h-50.133903c-4.270468 0-7.711411-3.502975-7.711412-7.655461v-92.972374c-23.553861-12.045127-39.868239-36.193764-39.868239-64.49732 0-40.045821 32.513207-72.618627 72.563893-72.618627 40.10542 0 72.623492 32.572806 72.623493 72.618627 0.053518 28.303555-16.259644 52.450976-39.817155 64.49732z" p-id="3131" fill="#1afa29"></path><path d="M932.254884 286.08241c0-80.218138 65.323194-145.541332 145.541332-145.541332 80.216921 0 145.424566 65.323194 145.424566 145.541332V437.738137h90.601784V282.229137c0-130.235275-105.850674-236.1419-236.083517-236.1419-130.234059 0-236.201499 105.906624-236.201499 236.1419V437.738137h90.717334V286.08241z" p-id="3132" fill="#1afa29"></path></svg></i>${inspectionDesc}`;
                    }else{//未手动解锁
                        return  `<i class="lock" style="vertical-align: super;font-size: 30px;"><svg t="1614132616909" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="6593" width="32" height="32"><path d="M721.92 493.24032l0-74.26048c0 0-0.02048-173.21984-179.2-173.21984-179.17952 0-179.2 173.21984-179.2 173.21984l0 74.26048L286.72 493.24032 286.72 839.68l512 0L798.72 493.24032 721.92 493.24032zM568.32 684.3392l0 81.1008-51.2 0 0-81.1008C501.88288 675.75808 491.52 659.968 491.52 641.69984c0-27.3408 22.9376-49.50016 51.2-49.50016 28.2624 0 51.2 22.13888 51.2 49.50016C593.92 659.968 583.55712 675.75808 568.32 684.3392zM670.72 493.21984l-256 0 0-73.91232c0.28672-20.72576 7.80288-124.04736 128-124.04736 120.17664 0 127.71328 103.3216 128 123.71968L670.72 493.21984z" p-id="6594" fill="#d81e06"></path></svg></i>${inspectionDesc}`;
            ...
            if(taskYear){//是否需要锁定
                if(inspectionDesc!=currentYear){ //判断是否为年
                    if(isLock==1){//不是本年
                        return  `<i class="unlock" style="vertical-align: super;font-size: 30px;"><svg t="1614130652256" class="icon" viewBox="0 0 1448 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3130" width="32" height="32"><path d="M868.057993 437.736921H419.565235c-32.212778 0-58.320892 26.105681-58.320892 58.322108v425.769637c0 32.27481 26.108114 58.382924 58.320892 58.382925h606.019618c32.21886 0 58.266158-26.108114 58.266158-58.382925v-425.769637c-0.004865-32.21886-26.103249-58.322109-58.326973-58.322108H868.057993z m-112.727696 282.304548L755.329081 813.013843c0 4.152485-3.437295 7.655461-7.655461 7.655461h-50.133903c-4.270468 0-7.711411-3.502975-7.711412-7.655461v-92.972374c-23.553861-12.045127-39.868239-36.193764-39.868239-64.49732 0-40.045821 32.513207-72.618627 72.563893-72.618627 40.10542 0 72.623492 32.572806 72.623493 72.618627 0.053518 28.303555-16.259644 52.450976-39.817155 64.49732z" p-id="3131" fill="#1afa29"></path><path d="M932.254884 286.08241c0-80.218138 65.323194-145.541332 145.541332-145.541332 80.216921 0 145.424566 65.323194 145.424566 145.541332V437.738137h90.601784V282.229137c0-130.235275-105.850674-236.1419-236.083517-236.1419-130.234059 0-236.201499 105.906624-236.201499 236.1419V437.738137h90.717334V286.08241z" p-id="3132" fill="#1afa29"></path></svg></i>${inspectionDesc}`;
                    }else{//未手动解锁
                        return  `<i class="lock" style="vertical-align: super;font-size: 30px;"><svg t="1614132616909" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="6593" width="32" height="32"><path d="M721.92 493.24032l0-74.26048c0 0-0.02048-173.21984-179.2-173.21984-179.17952 0-179.2 173.21984-179.2 173.21984l0 74.26048L286.72 493.24032 286.72 839.68l512 0L798.72 493.24032 721.92 493.24032zM568.32 684.3392l0 81.1008-51.2 0 0-81.1008C501.88288 675.75808 491.52 659.968 491.52 641.69984c0-27.3408 22.9376-49.50016 51.2-49.50016 28.2624 0 51.2 22.13888 51.2 49.50016C593.92 659.968 583.55712 675.75808 568.32 684.3392zM670.72 493.21984l-256 0 0-73.91232c0.28672-20.72576 7.80288-124.04736 128-124.04736 120.17664 0 127.71328 103.3216 128 123.71968L670.72 493.21984z" p-id="6594" fill="#d81e06"></path></svg></i>${inspectionDesc}`;
      ...
      getSearchStations(obj){
            ...
            if(sessionStorage.getItem('roleType').toString()=="3"){      
                ...
                {  
                    //开始时间
                     ...
                     if (month1 < 10)  month1 = '0' + month1
                     if (day1 < 10)  day1 = '0' + day1
                     self.queryparam.startDate = newdate.getFullYear() + '-' + month1 + '-' + day1  
                ...
                this.$http({
                method: 'GET',
                url: self.api+'/api/Yw_Task/GetEndTimeofMonthByPlan?choosedMonth='+self.choosedSearchMonth+'&flag='+ self.flag
            }).then(res => {
                if(res.status==200){
                    debugger;
       ...
       selectChangeRWLX(val, valObj) {
            ...
            this.rateRWLBSelect.selectUrl=this.api+'/api/Select/GetSelectList?TypeCode=002006&DictCode='+val;
       ...
       //任务类别
        ...
        handleBatchUnLock(ids){
            ...
            this.$http({
                method: 'GET',
                url: self.api+'/api/InspectionPlan/BatchUpdateYWTaskUnLock?taskNo='+ids
            }).then(res => {
                if(res.status==200){
                    self.getTaskList();
                    self.$message({
        ...
        handleBatchSuoD(){ 
            ...
            this.$confirm('确认锁定？').then(function () {
        ...
        handleBatchLock(ids){
            ...
            this.$http({
                method: 'GET',
                url: self.api+'/api/InspectionPlan/BatchUpdateYWTaskLock?taskNo='+ids
            }).then(res => {
                if(res.status==200){
                    self.getTaskList();
                    self.$message({
        ...
        handleBatchSD(){
            ...
            this.$http({
                method: 'GET',
                url: self.api+'/api/InspectionPlan/BatchSDTask?SStations='+SStations
            }).then(res => {
                if(res.status==200){
                    self.getTaskList();
                    self.$message({
        ...
        handleBatchCancel(){
            ...
            this.handleSelection.forEach((item,index) => {
                ...
                {
                    tips += "选中第" + (index+1) + "行不能撤回,下发后,但未完成的任务才能撤回!";
                }
            });
            if(tips!=''){
                self.$message({
                ...
                this.$confirm('确认撤回？').then(function () {
        ...
        handleAdd(){
            var obj = {taskNo:'',firstButtonType:'add',taskStatus:this.queryparam.Status,taskCategoryId:this.queryparam.TaskCategoryId,chooseStationIds:this.queryparam.chooseStationIds,chooseStartTime:this.queryparam.startDate,chooseEndTime:this.queryparam.endDate};
            this.$emit('jump',{param:'任务编辑',path:'/index/ywTaskDisplay?obj='+JSON.stringify(obj),isjump:true});
        },
        ...
        handleDetail(index,row){
            var obj = {taskNo:row.taskNo,firstButtonType:'Detail',taskStatus:this.queryparam.Status,taskCategoryId:this.queryparam.TaskCategoryId,chooseStationIds:this.queryparam.chooseStationIds,chooseStartTime:this.queryparam.startDate,chooseEndTime:this.queryparam.endDate};
            this.$emit('jump',{param:'任务编辑',path:'/index/ywTaskDisplay?obj='+JSON.stringify(obj),isjump:true});
        },
        ...
        handledeal(index, row){
            var obj = {taskNo:row.taskNo,firstButtonType:'Deal',taskStatus:this.queryparam.Status,taskCategoryId:this.queryparam.TaskCategoryId,chooseStationIds:this.queryparam.chooseStationIds,chooseStartTime:this.queryparam.startDate,chooseEndTime:this.queryparam.endDate};
            this.$emit('jump',{param:'任务编辑',path:'/index/ywTaskDisplay?obj='+JSON.stringify(obj),isjump:true});
        },
         
         //任务审核
        handleAudit(index, row){
            var obj = {taskNo:row.taskNo,firstButtonType:'Audit',taskStatus:this.queryparam.Status,taskCategoryId:this.queryparam.TaskCategoryId,chooseStationIds:this.queryparam.chooseStationIds,chooseStartTime:this.queryparam.startDate,chooseEndTime:this.queryparam.endDate};
            this.$emit('jump',{param:'任务编辑',path:'/index/ywTaskDisplay?obj='+JSON.stringify(obj),isjump:true});
        },
        ...
        handleArrangement(){
            ...
            this.handleSelection.forEach((item,index) => {
               if(item.status!=1){
                  tips += "选中第" + (index+1) + "行不能下发,存在状态不为待下发的数据，请核对!";
               }
               else{
                 if (index != self.handleSelection.length - 1) {
                    ids += item.taskNo + ','
            ...
            if(tips!=''){
               ...
               this.$confirm('确认下发？').then(function () {
        ...
        getTaskList(obj){
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/Yw_Task/GetTaskList?pagesize=' + self.page.pageSize + '&pageindex=' + self.page.pageNo+ '&TaskNo=' 
                + self.queryparam.TaskNo+ '&TaskType=' + self.queryparam.TaskType+ '&TaskCategoryId=' 
                + self.queryparam.TaskCategoryId+ '&Statuss=' + strStatus+ '&startDate=' 
                + self.queryparam.startDate+ '&endDate=' + self.queryparam.endDate +'&SStations=' 
                + self.queryparam.chooseStationIds+'&YJType='+ self.queryparam.YJType
            }).then(res => {
                if(res.status==200){
        ...
        getParam() {
            ...
            if(JSON.parse(data)!=null){
                ...
                if(JSON.parse(data).taskCategoryId!=undefined){
                    self.queryparam.TaskCategoryId = JSON.parse(data).taskCategoryId.toString();  
                }
                ...
                //任务状态
                if(JSON.parse(data).taskStatus!=undefined && JSON.parse(data).taskStatus.toString()!="")
                {
        ...
        download(obj){
            ...
            this.$http({
                ...
                url:  this.api+'/api/Yw_Task/GetTaskListDownLoad?pagesize=' + self.page.pageSize + '&pageindex=' + self.page.pageNo+ '&TaskNo=' 
                + self.queryparam.TaskNo+ '&TaskType=' + self.queryparam.TaskType+ '&TaskCategoryId=' 
                + self.queryparam.TaskCategoryId+ '&Status=' + self.queryparam.Status+ '&startDate=' 
                + self.queryparam.startDate+ '&endDate=' + self.queryparam.endDate +'&SStations=' 
                + self.queryparam.chooseStationIds+'&YJType='+ self.queryparam.YJType
            }).then(res => {
                if(res.status==200){              
    ...
    components:{
        treeSStation,rateTable,rateUpload,rateSelect
    },
    created(){
    },
    mounted() {
...
.el-aside {color: #333;}
.el-header{height: 120px !important;}
