                    ...
                    <div class="search">
                        <el-form :inline="true" class="demo-form-inline">  
                          ...
                          <!--<el-form-item label="任务类别：">
                                <el-select v-model='queryparam.TaskType'   placeholder='请选择任务类别' style="width: 100%;">
                          ...
                          <el-form-item label="创建日期：">
                            ...
                            <el-form-item class="btn">
                                <el-button type="primary" icon="el-icon-search" v-has="'ywFourthInspectionJudge_handleSearch'"   @click="getList();">查询</el-button>
                            </el-form-item>
                        </el-form>
                    </div>
                    <div class="tools">  
...
export default {
      ...
      rateSelectTaskType:{
        ...
        page:{   //关于页码的相关参数
            ...
            columns: [
              ...
              {prop: 'show_DealTime', label: '检查时间', align: 'center',isShow:true },
              ...
              list: [
                    {
                      ...
                      hasbutton:'ywFourthInspectionJudge_handleSure',
                      ...
                      show: true,   //是否显示按钮                      
                      className:'success', //样式类名
                      hasbutton:'ywFourthInspectionJudge_handleDetail',
                      disabled: false,   //是否禁用按钮 默认是danger的禁用样式
                      method: (index, row) => {
    ...
    methods:{     
        ...
        handleOperate (index, row) {  //操作栏编辑按钮
            var self=this;
            let obj = { taskNo: row.taskNo,type: '',fromTab:'operate',taskStatus:row.status};              
            this.$emit('jump',{param:'第四方检查任务窗口',path:'/index/ywFourthInspectionTaskShow?obj='+JSON.stringify(obj),isjump:true});  
            //关闭当前tab
            this.$emit("closeCurrentPage", "第四方任务整改"); //调用closeCurrentPage方法注意；两点1：第二个参数必须是string，2：长度必须大于等于4       
        },
        
        //查看
        handleView(index, row){
            var self=this;
            let obj = { taskNo: row.taskNo,type: 'view',fromTab:'operate',taskStatus:row.status };              
            this.$emit('jump',{param:'第四方检查任务窗口',path:'/index/ywFourthInspectionTaskShow?obj='+JSON.stringify(obj),isjump:true});  
            //关闭当前tab
            this.$emit("closeCurrentPage", "第四方任务整改"); //调用closeCurrentPage方法注意；两点1：第二个参数必须是string，2：长度必须大于等于4 
        },
        ...
        getList(obj){
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/FourthInspectionTask/GetInspectionTaskJudgeByPage?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+'&SStation='+self.queryparam.chooseStationIds+ ' &StartDate=' + self.queryparam.StartDate + '&EndDate=' + self.queryparam.EndDate +'&TaskType=' + self.queryparam.TaskType +' &TaskNo=' + self.queryparam.TaskNo +'&TaskCategoryId='+self.queryparam.TaskCategoryId +'&Status=' + self.queryparam.Status
            }).then(res => {
                if(res.status==200){
