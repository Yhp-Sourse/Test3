                    ...
                    <div class="search">
                        <el-form :inline="true" class="demo-form-inline" >
                            <el-form-item label="运维人员：">
                                <el-input v-model="queryparam.RealName" placeholder="输入姓名"></el-input>
                            </el-form-item>
                            ...
                            <el-form-item class="btn">
                                <el-button type="primary" v-has="'ywSignin_handleSearch'" icon="el-icon-search"  @click="getList();">查询</el-button>
...
export default {
    ...
    data() {
      return {
        ...
        columns: [
          {prop: 'finishTime',label: '任务时间周期',align: 'center',width:180,isShow:true,formatter:this.timePeriod},       
          {prop: 'city',label: '城市',align: 'center',isShow:true},//formatter重构//return `返回参数`
          {prop: 'sStationName',label: '站点',align: 'center',isShow:true,width:160},
          ...
          {prop: 'taskCategoryName',label: '任务类型',align: 'center',isShow:true},
          {prop: 'signIn',label: '签到时间 ',align: 'center',isShow:true,width:200},
          {prop: 'signOut',label: '签退时间  ',align: 'center',isShow:true,width:200},
          {prop: 'hasHousr',label: '分钟',align: 'center',isShow:true},
          {prop: 'remark',label: '工作描述',align: 'center',isShow:true}, 
          {prop: 'inPhoto',label: '照片',align: 'center',isShow:true,width:100,
            // formatter: (row, column, cellValue) => {
            //     //return "<el-image style='width: 40px; height: 40px' src='"+row.inPhotoUrl+"' fit='fit'></el-image>"
            //     return "<p><img width=40px; src='"+row.inPhotoUrl+"' /></p>"
            // }
            render:(h,params)=>{
                // var arr=[params.row.inPhotoUrl];
                return h('el-image',{//如果要使用其他组件，将el-image替换其他组件 列如：Esign  props:{imgid:params.row.inPhotoUrl}//imgid是自定义的值
                    props: {
          ...
          list: [
             {
               ...
               method: (index, row) => {
                   this.downloadFile(row.sStationName+row.realName+row.signIn+'.png', row.inPhotoUrl)
               }
           }
            ]
        }, // 列操作按钮
    ...
    methods:{
      ...
      getWeekOfYear(today) {
        ...
        handleDetail (index, row) {  //操作栏编辑按钮
            var self=this;
        ...
        getList(obj){
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/Yw_UsrSignIn/GetList?pagesize=' + self.page.pageSize + '&pageindex=' + self.page.pageNo+ '&startDate=' 
                + self.queryparam.startDate+ '&SStations=' + self.queryparam.chooseStationIds+'&endDate=' + self.queryparam.endDate +'&RealName=' + self.queryparam.RealName 
            }).then(res => {
                if(res.status==200){
        ...
        download(obj){
            ...
            this.$http({
                ...
                url: this.api+'/api/Yw_UsrSignIn/GetListDownLoad?pagesize=' + self.page.pageSize + '&pageindex=' + self.page.pageNo+ '&startDate=' 
                + self.queryparam.startDate+ '&SStations=' + self.queryparam.chooseStationIds+'&endDate=' + self.queryparam.endDate +'&RealName=' + self.queryparam.RealName 

            }).then(res => {
                if(res.status==200){              
                let blob = new Blob([res.data], {type: 'application/vnd.ms-excel' })
                const fileName = self.queryparam.startDate+'至'+self.queryparam.endDate+self.queryparam.RealName+'考勤签到.xls';
...
::v-deep .el-image{height: 45px !important;}
#v_Yw_UsrSignIn >>> .el-image{height: 45px !important;}
</style>