                    ...
                    <div class="search">
                        <el-form :inline="true" class="demo-form-inline">
                            <el-form-item label="计划编号：">
                                <el-input v-model="queryparam.PlanNo" placeholder="请输入计划编号："></el-input>
                            </el-form-item>
...
export default {
    ...
    methods:{     
        ...
        handleOperate (index, row) {  //操作栏编辑按钮
            ...
            this.$emit('jump',{param:'检查计划窗口',path:'/index/ywInspectionPlanAdd?obj='+JSON.stringify(obj),isjump:true});  
            //关闭当前tab
            this.$emit("closeCurrentPage", "检查计划"); //调用closeCurrentPage方法注意；两点1：第二个参数必须是string，2：长度必须大于等于4       
        },
        ...
        getList(obj){      
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/InspectionPlan/AlarmstatisticsByStation?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+' &Status='+self.queryparam.Status+' &PlanNo='+self.queryparam.PlanNo+' &StartDate='+self.queryparam.StartDate+'&EndDate=' + self.queryparam.EndDate
            }).then(res => {
                if(res.status==200){
