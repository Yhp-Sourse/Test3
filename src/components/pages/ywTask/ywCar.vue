...
import rateTable  from '@/components/pages/common/rateTable'    //引入rateTable组件
import rateCascader from '../common/rateCascader' //引入rateCascader组件
import rateUpload from '@/components/pages/common/rateUpload' //引入rateCascader组件
...
export default {
    ...
    methods:{
        ...
        async getList(obj){
            ...
            this.$store.dispatch('action1',"有效期是8个小时。");
            var self = this;
            let res = await this.$httpPromise.get(this.api+'/api/Yw_Vehicle/GetList?pagesize=' + self.page.pageSize + '&pageindex=' + self.page.pageNo+ '&startDate=' + self.queryparam.startDate + '&endDate=' + self.queryparam.endDate+ '&Licenseplate=' + self.queryparam.Licenseplate);
            self.list=res.data;
