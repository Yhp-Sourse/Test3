    ...
    <div id="v_deviceOnlineInfo">
                    ...
                    <div class="search">
                        <el-form :inline="true" class="demo-form-inline">
                            <el-form-item label="绑定账号：">
                                <el-input v-model="queryparam.UsrName" placeholder="请绑定账号"></el-input>
                            </el-form-item>

                            <el-form-item label="昵称(备注)：">
                                <el-input v-model="queryparam.HoneyName" placeholder="请填写昵称"></el-input>
                            </el-form-item>
                                                       
                            <el-form-item label="状 态：" >
                                   <el-select v-model='queryparam.Status'   placeholder='请选择状态' style="width: 100%;">
                                       <el-option v-for='item in StatusOptions' :key='item.value' :label='item.label' :value='item.value'></el-option>
                                   </el-select>
                            </el-form-item>

                            <el-form-item class="btn">
                                <el-button type="primary" icon="el-icon-search"  v-has="'wxts_handleSearch'"  @click="getList();">查询</el-button>
                            </el-form-item>                           
                        </el-form>
                    </div>
                    <div class="tools">
                         <el-button size="small" icon="el-icon-refresh"   class=" el-button--iconButton" v-has="'wxts_handleAnSync'"   style="text-overflow: initial;" @click="handleSync">同步</el-button> 
                         <el-button size="small"  icon="el-icon-s-promotion"  class=" el-button--iconButton" v-has="'wxts_handleDailySend'"   style="text-overflow: initial;" @click="handleSendReport">日报推送</el-button>
                    </div>
                </el-header>
        ...
        <div>
                        ...
                        <el-col :span="24">
                            <div>
                                <el-form-item label="推送类型：" prop="pushTypes">                                 
                                   <el-select v-model="ruleForm1.pushTypes" multiple placeholder="请选择推送类型">
                                      <el-option v-for="item in pushType" :key="item.id" :label="item.name" :value="item.id"></el-option>
                                   </el-select>
                                </el-form-item>
                            </div>
                        </el-col>
                       ...
                       <el-col :span="24">
                            <div> 
                                <el-form-item label="推送站点：" prop="sStations">
                                     <rateCascader style="overflow:hidden;" :checkStrictly='false' :selectOption='ruleForm1.sStations' @selectOptionEvent="selectOptionEvent"></rateCascader>
                                </el-form-item>
                            </div>
                        </el-col>                    
                    </el-row>
                       ...
                       <el-col :span="12">
                            <div> 
                                <el-form-item label="推送状态：" prop="isPush">
                                   <el-select v-model='ruleForm1.isPush'   placeholder='请选择推送状态' style="width: 100%;">
                                       <el-option v-for='item in pushStatusOptions' :key='item.value' :label='item.label' :value='item.value'></el-option>
                                   </el-select>
                                </el-form-item>
                            </div>
                        </el-col>                    
            ...
            <el-dialog
                ...
                >
                <el-form :model="ruleForm3" status-icon :rules="rules3" ref="ruleForm3" label-width="100px" class="demo-ruleForm" size="mini">
                   
                    <el-row>
                         <el-col :span="24">
                            <div>
                                <el-form-item label="内容：" prop="content">
                                    <el-input v-model="ruleForm3.content" autocomplete="off" type="textarea" :rows="4"></el-input>
                                </el-form-item>
                            </div>
                          </el-col>
                    </el-row>
            ...
            <el-dialog
                :title="digTitle"
                :visible.sync="dialogVisibleSendReport"
                width="50%"
                :before-close="handleSendDailyReportClose"
                >
                <el-form :model="ruleForm4" status-icon :rules="rules4" ref="ruleForm4" label-width="100px" class="demo-ruleForm" size="mini">
                   <el-row>                      
                         <el-col :span="12">
                            <div>
                                <el-form-item label="日报日期：" prop="choseYear">
                                    <el-date-picker
                                               v-model="ruleForm4.choseYear"
                                               type="date"
                          ...
                          <el-col :span="12">
                            <div>                               
                                 <el-button size="small" type="primary" class=" el-button--iconButton"  style="text-overflow: initial;" @click="handlSearch()">查询</el-button> 
                                 <el-button size="small" type="primary" class=" el-button--iconButton"  style="text-overflow: initial;" @click="handlSendDailyReport()">发送</el-button> 
                            </div>
                         ...
                         <el-col :span="24">
                            <div>
                                <el-form-item label="推送内容：" prop="reportContent">
                                    <el-input v-model="ruleForm4.reportContent" autocomplete="off" type="textarea" :rows="6"></el-input>
                                </el-form-item>
                            </div>
                          </el-col>
                    </el-row>
...
export default {
    ...
    data() {   
      return {  
       ...
       pushStatusOptions: [{
        ...
        rules1:{
            pushTypes: [{ required: true, message: '请选择推送类型', trigger: 'blur' }],
            sStations: [{ required: true, message: '请选择站点', trigger: 'blur' }],
            isPush: [{ required: true, message: '请选择推送状态', trigger: 'blur' }],  
        },
        rules3:{},
        rules4:{
           choseYear:'',
        ...
        page:{   //关于页码的相关参数
            ...
            columns: [
              ...
              {prop: 'headimgurl', label: '微信头像',align: 'center',isShow:false,formatter: (row, column, cellValue) => { return  "<img src= "+row.headimgurl+"  width=50,heigth=50 ></img> "} },
              {prop: 'userName', label: '绑定账号',align: 'center',isShow:true },
              {prop: 'groupName', label: '分组名称',align: 'center',isShow:false },
              {prop: 'pushTypeName', label: '推送类别',align: 'center',isShow:true },
              {prop: 'sStationNames', label: '站点名称',align: 'center',isShow:true },
    ...
    methods:{
       ...
       handlSearch(){
           ...
           this.$http({
                method: 'get',
                url: self.api+'/api/Yw_WeChatPush/GetDailyReportByTime?queryTime=' + self.ruleForm4.choseYear
            }).then(res => {
                if(res.status==200){              
        ...
        handleSendMessage(){
            ...
            this.$http({
                method: 'post',
                url: this.api+'/api/Yw_WeChatPush/SendMessage?openId=' + self.openId + '&content='+ self.ruleForm3.content
            }).then(res => {
                debugger;
                if(res.status==200){
            ...
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.$http({
                        ...
                        method: 'post',
                        url: self.api+'/api/Yw_WeChatPush/CheckingUsrInfo?openId='+self.openId,
        ...
        handleConfigSave(){
           ...
           this.$http({
                method: 'GET',
                url: self.api+'/api/Yw_WeChatPush/SaveWeChatPushConfig?jsonStr='+ jsonStr
            }).then(res => {
                if(res.status==200){                
        ...
        getList(obj){      
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/Yw_WeChatPush/GetWeChatPushPageList?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+'&usrName='+self.queryparam.UsrName+'&nickName='+self.queryparam.HoneyName +'&isPush=' + self.queryparam.Status
            }).then(res => {
                if(res.status==200){
