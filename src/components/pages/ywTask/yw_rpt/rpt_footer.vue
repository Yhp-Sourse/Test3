<template>
    <div>
        <div class="beizhu" v-if="actionStatus.yijian">审核意见：<el-input  type="textarea" :rows="3" v-model="footerData.isFinished" autocomplete="off"></el-input></div>
        ...
        <div id="btn" style="margin: 20px 0; text-align: center">
            <el-button v-if="actionStatus.sava" @click='rpt_sava' type="primary" round >保存</el-button>
            <el-button v-if="actionStatus.submit" @click='rpt_submit' type="success" round>确认</el-button>
            <el-button v-if="actionStatus.success" @click='rpt_success' type="primary" round >通过</el-button>
            <el-button v-if="actionStatus.refuse" @click='rpt_refuse' type="success" round>拒绝</el-button>
        </div>
    </div>
</template>
<script>
export default {
    ...
    methods:{
        ...
        rpt_sava(){
            //保存状态为 0
            this.$emit('rpt_sava', this.footerData);
        },
        rpt_submit(){
            //提交状态为 1
            this.$emit('rpt_submit', this.footerData);
        },
        rpt_success(){
            //通过状态为2
             this.$emit('rpt_success', this.footerData);
        },
        rpt_refuse(){
            //拒绝状态回退为3
            this.$emit('rpt_refuse', this.footerData);
        }
    }
}
</script>
