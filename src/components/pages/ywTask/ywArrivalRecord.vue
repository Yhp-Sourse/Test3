...
export default {
    ...
    data() {
      return {
        ...
        columns: [
          {prop: 'sStationName',label: '站点名称',align: 'center',isShow:true,width:160},
          {prop: 'show_SETime',label: '进站时间',align: 'center',isShow:true,width:320},
          {prop: 'stateName',label: '状态',align: 'center',isShow:true,},
          {prop: 'reason',label: '进站原因',align: 'center',isShow:true},
          {prop: 'remarks',label: '备注',align: 'center',isShow:true},
          {prop: 'fileName',label: '附件',align: 'center',isShow:true,width:200,
          ...
          {prop: 'auditorName',label: '审核人',align: 'center',isShow:true},
          {prop: 'auditorTime',label: '审核时间',align: 'center',isShow:true,width:200,formatter(row, column, cellValue){
              if(row.auditorTime){
                  return row.auditorTime.replace("T"," ");
              }
          }},
          {prop: 'auditorReason',label: '审核意见',align: 'center',isShow:true,width:200},
          {prop: 'createByName',label: '创建人',align: 'center',isShow:true,width:100},
          {prop: 'createTime',label: '创建时间',align: 'center',isShow:true,width:200,formatter(row, column, cellValue){
              if(row.createTime){
        ...
        operates: {   //操作栏
          ...
          list: [
            ...
            },
            {
                ...
                bgColortype:'warning',
                className:'searchAll',
                hasbutton:'arrivalRecord_handleCheck',//按钮权限控制
                disabled: false,
                method: (index, row) => {
                    if(row.status==1){
                        this.$message({
    ...
    methods:{
       getSearchStations(obj){
        ...
        getList(obj){
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/Yw_ArrivalLog/GetList?pagesize=' + self.page.pageSize + '&pageindex=' + self.page.pageNo+ '&SStation='+self.queryparam.chooseStationIds+' &startDate=' + self.queryparam.startDate + '&endDate=' + self.queryparam.endDate+ '&Status=' + self.queryparam.Status
            }).then(res => {
                if(res.status==200){
                    self.list=res.data.data;
                    self.page.total = res.data.count;
        ...
        download(obj){
            ...
            this.$http({
                ...
                url: this.api+'/api/Yw_ArrivalLog/GetListDownLoad?pagesize=' + self.page.pageSize + '&pageindex=' + self.page.pageNo+ '&SStation= '+ self.queryparam.chooseStationIds + ' &startDate=' + self.queryparam.startDate + '&endDate=' + self.queryparam.endDate+ '&Status=' + self.queryparam.Status
            }).then(res => {
                if(res.status==200){              
    let blob = new Blob([res.data], {type: 'application/vnd.ms-excel' })
    let  downTime= self.downLoadDate();
    const fileName = downTime +'-进站记录.xls';
