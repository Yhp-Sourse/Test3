                    ...
                    <div class="search">
                        ...
                        <el-form-item label="创建日期：">
                            ...
                            <el-form-item class="btn">
                                <el-button type="primary" icon="el-icon-search"  v-has="'ywProviceInspectionTask_handleSearch'"   @click="getList();">查询</el-button>
                                <el-button type="primary" icon="el-icon-download"  v-has="'ywProviceInspectionTask_handleExport'"   @click="download();">导出</el-button>
                            </el-form-item>
                        </el-form>
                    </div>
                    <div class="tools">
                         <el-button size="small" class=" el-button--iconButton" icon="el-icon-plus" v-has="'ywInternalInspectionTask_handleAdd'"  style="text-overflow: initial;" @click="handleAdd">添加</el-button> 
                         <el-button size="small" class=" el-button--iconButton" icon="el-icon-s-promotion" v-has="'ywProviceInspectionTask_handleArrangement'"  style="text-overflow: initial;" @click="handleArrangement">领取</el-button>
                         <el-button size="small" class=" el-button--iconButton" icon="el-icon-delete" v-has="'ywProviceInspectionTask_handleMultiplDel'"  style="text-overflow: initial;" @click="handleMultiplDel">删除</el-button>
                         <el-button size="small" class=" el-button--iconButton" icon="el-icon-delete" v-has="'ywProviceInspectionTask_handleRecall'"  style="text-overflow: initial;" @click="handleBatchCancel">撤回</el-button>
                    </div>
                </el-header>
...
export default {
      ...
      return {         
        ...
        page:{   //关于页码的相关参数
            ...
            columns: [
              ...
              {prop: 'show_GenModifyProblem', label: '省站是否确认', align: 'center',isShow:true },
              ...
              list: [
                    {
                      ...
                      hasbutton:'ywProviceInspectionTask_handleEdit',
                      ...
                      hasbutton:'ywProviceInspectionTask_handleOperate',
                      disabled: false,   //是否禁用按钮 默认是danger的禁用样式
                      method: (index, row) => {
                        var self = this;
                        if(row.status==1){
                      ...
                      show: true,   //是否显示按钮                      
                      className:'info', //样式类名
                      hasbutton:'ywProviceInspectionTask_handleDetail',
                      disabled: false,   //是否禁用按钮 默认是danger的禁用样式
                      method: (index, row) => {
    ...
    methods:{      
        ...
        handleOperate (index, row) {  //操作栏编辑按钮
            var self=this;
            let obj = { taskNo: row.taskNo,type: '',fromTab:'' };              
            this.$emit('jump',{param:'省站检查任务窗口',path:'/index/ywProviceInspectionTaskShow?obj='+JSON.stringify(obj),isjump:true});  
            //关闭当前tab
            this.$emit("closeCurrentPage", "省站检查任务"); //调用closeCurrentPage方法注意；两点1：第二个参数必须是string，2：长度必须大于等于4       
        },
        
        //查看
        handleView(index, row){
            var self=this;
            let obj = { taskNo: row.taskNo,type: 'view', fromTab:'' };              
            this.$emit('jump',{param:'省站检查任务窗口',path:'/index/ywProviceInspectionTaskShow?obj='+JSON.stringify(obj),isjump:true});  
            //关闭当前tab
            this.$emit("closeCurrentPage", "省站检查任务"); //调用closeCurrentPage方法注意；两点1：第二个参数必须是string，2：长度必须大于等于4 
        },
       ...
       ArrangeTask(ids){
        ...
        handleBatchCancel(){
            ...
            this.handleSelection.forEach((item,index) => {
                ...
                {
                    tips += "选中第" + (index+1) + "行不能撤回,领取后的任务才能撤回!";
                }
            });
            if(tips!=''){
                self.$message({
    ...
    const fileName = downTime +'-省内检查任务.xls';
