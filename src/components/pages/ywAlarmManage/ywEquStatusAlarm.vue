                    ...
                    <div class="search">
                        <el-form :inline="true" class="demo-form-inline">                   
                            <el-form-item label="处理状态：">
                                <el-select v-model='queryparam.DealStatus'   placeholder='选择处理状态' >
                                       <el-option v-for='(item, index) in this.AlarmStatusOptions' :key='index' :label='item.label' :value='item.value'></el-option>
                                 </el-select>                               
                             </el-form-item>

                            <el-form-item label="设备名称：">
                                <el-input v-model="queryparam.DevName" placeholder="设备名称"></el-input>
                            </el-form-item>     
                         
                            <el-form-item label="状态量">
                                <el-input v-model="queryparam.StateName" placeholder="状态量"></el-input>
                            </el-form-item>   
                          

                            <el-form-item label="报警类型：">
                                <el-select v-model='queryparam.AlarmType'   placeholder='选择报警类型' style="width: 100%;">
                                       <el-option v-for='(item, index) in this.AlarmTypeOptions' :key='index' :label='item.label' :value='item.value'></el-option>
                                </el-select>                               
                            </el-form-item>
                                          
                            <el-form-item label="是否恢复：">
                                <el-select v-model='queryparam.Resumed'   placeholder='选择是否恢复' style="width: 100%;">
                                       <el-option v-for='(item, index) in this.ResumedOptions' :key='index' :label='item.label' :value='item.value'></el-option>
                                </el-select>                               
                            </el-form-item>
                            ...
                            <el-form-item class="btn">
                                <el-button type="primary" icon="el-icon-search"  v-has="'equStatusAlarm_handleSearch'"   @click="getList();">查询</el-button>
                            </el-form-item>
                        </el-form>
                    </div>
                        ...
                        <el-col :span="12">
                            <div>
                                <el-form-item label="处理结果：" prop="DealStatus">
                                     <el-select v-model='ruleForm.DealStatus'   placeholder='选择处理结果' >
                                       <el-option v-for='(item, index) in this.AlarmDealOptions' :key='index' :label='item.label' :value='item.value'></el-option>
                                     </el-select>       
                                </el-form-item>
                            </div>
                        </el-col>                     
                        ...
                        <el-col :span="24">
                             <div>
                                <el-form-item label="处理情况：" prop="DealingRemark">
                                   <el-input v-model="ruleForm.DealingRemark" autocomplete="off" type="textarea" :rows="6"></el-input>
                                </el-form-item>
                            </div>                         
                        </el-col>                                       
                    </el-row>
                ...
                <el-form :model="ruleForm1" status-icon :rules="rules1" ref="ruleForm1" label-width="100px" class="demo-ruleForm" size="mini">
                    <el-row>
                        <el-col :span="12">
                            <div>
                                ...
                                <el-form-item label="处理结果：" prop="DealStatus">
                                     <el-select v-model='ruleForm1.DealStatus'  disabled  placeholder='选择处理结果' >
                                       <el-option v-for='(item, index) in this.AlarmDealOptions' :key='index' :label='item.label' :value='item.value'></el-option>
                                     </el-select>       
                                </el-form-item>
                            </div>
                    ...
                    <el-row>                                         
                        <el-col :span="24">
                             <div>
                                <el-form-item label="处理情况：" prop="DealingRemark">
                                   <el-input v-model="ruleForm1.DealingRemark" autocomplete="off" type="textarea" :rows="6" :disabled="true" ></el-input>
                                </el-form-item>
                            </div>                         
                        </el-col>                                       
                    </el-row>                  
    ...
    data() {
      ...
      return {  
        ...
        page:{   //关于页码的相关参数
            ...
            columns: [
              {prop: 'show_DealStatus', label: '处理状态', width:120,align: 'center',isShow:true },
              {prop: 'unitName', label: '运维单位', width:120,align: 'center',isShow:true },
              {prop: 'city', label: '城市', align: 'center',isShow:true },
              {prop: 'townName', label: '区县', width:180, align: 'center',isShow:true },
              {prop: 'sStationName', label: '站点名称', align: 'center',isShow:true },
              {prop: 'show_SDatetime', label: '报警时间',width:120, align: 'center',isShow:true },
              ...
              {prop: 'show_AlarmType', label: '报警类型', align: 'center',isShow:true },
               {prop: 'show_Resumed', label: '是否恢复', align: 'center',isShow:true },
              {prop: 'show_ReSDatetime', label: '恢复时间', align: 'center',isShow:true },
            {prop: 'reValue', label: '恢复时的值', align: 'center',isShow:true },
              {prop: 'reupper_limit', label: '恢复时的上限值', align: 'center',isShow:true },
             {prop: 'relower_limit', label: '恢复时的下限值', align: 'center',isShow:true }
    ...
    methods:{
       ...
       //提交
        submitForm(formName) {
          ...
          //处理
        handleOperate (index, row) {  //操作栏处理按钮
            var self=this;
         ...
         //查看详情
        handleDetail (index, row) {  //操作栏详情按钮
            var self=this;
            self.digTitle="处理详情";
            this.getAlarmResultById(row.sStation,row.sDatetime,row.devCode,row.paramId);
        ...
        getAlarmResultById(stationId,sDatetime,devCode,paramId){
            var self = this;
                this.$http({
                method: 'GET',
                url: this.api+'/api/AlarmRemind/GetAlarmResultByInfo?sstation=' +stationId+ '&sDatetime=' + sDatetime+' &devCode='+devCode+'&paramId=' + paramId
            }).then(res => {
                if(res.status==200){
     ...
     getNowTime() {
       ...
       this.$set(this.queryparam, "StartTime", defaultDate);
        ...
        getList(obj){
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/AlarmRemind/GetAllEquStatusMessage?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+' &SStation='+staionId+ ' &StartDate=' + self.queryparam.StartTime + '&EndDate=' + self.queryparam.EndTime +' &AlarmType=' + self.queryparam.AlarmType +' &DevName=' + self.queryparam.DevName +' &Resumed=' + self.queryparam.Resumed +' &StateName=' + self.queryparam.StateName +' &DealStatus=' + self.queryparam.DealStatus
            }).then(res => {
                if(res.status==200){
