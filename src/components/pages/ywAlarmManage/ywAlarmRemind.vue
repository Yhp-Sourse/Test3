                    ...
                    <div class="search">
                        <el-form :inline="true" class="demo-form-inline">
                         ...
                         <el-form-item label="处理状态：">
                                <el-select v-model='queryparam.AlarmStatus'   placeholder='请选择状态类型' style="width: 100%;">
                         ...
                         <el-form-item label="报警日期：">
                            ...
                            <el-form-item class="btn">
                                <el-button type="primary" icon="el-icon-search"  v-has="'alarmRemind_handleSearch'"   @click="getList();">查询</el-button>
...
export default {
    ...
    data() {
      ...
      return {  
        ...
        rateSelectUnit:{          
            model: '',
            selectUrl:this.api +'/api/Common/GetListByDic?type=004001',
        ...
        page:{   //关于页码的相关参数
            ...
            columns: [
              {prop: 'newsId', label: '消息id', align: 'center',isShow:false },
              {prop: 'info', label: '报警类型', width:120,align: 'center',isShow:true },
              {prop: 'show_Status', label: '处理状态', width:120,align: 'center',isShow:true },
              {prop: 'cityName', label: '城市', align: 'center',isShow:true },
    ...
    watch: {
        NewType(newV,oldV) {
            // do something
            this.queryparam.AlarmType=newV;
      ...
      getSearchStations(obj){
       ...
       this.$set(this.queryparam, "StartTime", defaultDate);
        ...
        getList(obj){
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/AlarmRemind/GetAllAlarmMessage?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+' &SStaion='+self.queryparam.chooseStationIds+ ' &StartDate=' + self.queryparam.StartTime + '&EndDate=' + self.queryparam.EndTime +' &NewType=' + self.queryparam.AlarmType +'   &Status=' + self.queryparam.AlarmStatus
            }).then(res => {
                if(res.status==200){
        ...
        download(){
            ...
            this.$http({
                ...
                url: this.api+'/api/AlarmRemind/GetAllAlarmMessageDownLoad?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+' &SStaion='+self.queryparam.chooseStationIds+ ' &StartDate=' + self.queryparam.StartTime + '&EndDate=' + self.queryparam.EndTime +' &NewType=' + self.queryparam.AlarmType +'   &Status=' + self.queryparam.AlarmStatus
            }).then(res => {
                if(res.status==200){              
    let blob = new Blob([res.data], {type: 'application/vnd.ms-excel' })
    let  downTime= self.downLoadDate();
    const fileName = downTime +'-报警提醒.xls';
    ...
    update(){
        //数据更新时
        // this.queryparam.AlarmType=this.NewType;
    },
    mounted() {
