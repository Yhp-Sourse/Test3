<template>
    <div id="v_upload">
        <el-upload
            ...
            :limit='limit'
            :action="action+'?BusinessId='+BusinessId+'&BusinessType='+BusinessType+'&SStation='+SStation+'&CreatedTime='+CreatedTime+'&formType='+formType"
            :headers='headers'
            ...
            <div class="el-upload__text" style="font-size:16px;color:#fff;"><span v-html='title'></span></div>
            <span style="font-size:16px;color:#fff;">{{isUpload?"(已上传)":"(未上传)"}}</span>
            <div class="el-upload__tip" slot="tip"><span v-html='isUpload?filelist:""'></span><i v-if='isUpload' @click='handleuplodRemove(SStation,BusinessId,CreatedTime)' class="el-icon-close"></i></div>
        </el-upload>
        <div v-show='false'>
...
<script>
  export default {
    props:['CreatedTime','BusinessId','BusinessType','SStation','Ismultiple','limit','title','isUpload','formType','filelist'],
    data() {
      return {
        Base64:'',
        fileName:'',
    ...
    methods: {
      ...
      handleuplodRemove(SStation,BusinessId,CreatedTime){
        this.$emit('handleuplodRemove', {SStation,BusinessId,CreatedTime});
      }
    }
  }
</script>
