<template>
    <div id="v_upload">
        <el-upload
            ...
            <div class="el-upload__text">上传不超过30M的文件</div>
            <!-- 使用作用域插槽 -->
            <div class="el-upload__tip" slot="tip" v-if="todos && todos[0].fileName">
                <div v-for="(todo,index) in todos" :key="index" class="filelist">
                  <el-link type="primary" ><slot :todo="todo"></slot></el-link><i class="el-icon-close" @click="delFile(todo.QueId)"></i>
                </div>
            </div>
             <!-- 不使用作用域插槽
             <div class="el-upload__tip" slot="tip" v-if="todos[0].fileName">
                <div v-for="(todo,index) in todos" :key="index" class="filelist">
                  <el-link type="primary" >{{todo.fileName}}</el-link><i class="el-icon-close" @click="delFile(todo.QueId)"></i>
                </div>
            </div> -->
...
<script>
  export default {
    props:['BusinessId','BusinessType','Ismultiple','limit','todos'],
    data() {
      return {
        Base64:'',
        fileName:'',
