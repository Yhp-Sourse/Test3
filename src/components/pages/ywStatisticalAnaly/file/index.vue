                ...
                <el-header>
                    ...
                    <div class="tools">
                       <span v-show="false">当前路径：{{BusinessType}} </span>
                        ...
                        <el-button size="small" class="el-button-iconButton" icon="el-icon-upload" style="text-overflow: initial;" v-has="'fileMgr_handleUpload'" @click="handleUpload">上传</el-button>
                        <el-button size="small" class="el-button-iconButton" icon="el-icon-plus" style="text-overflow: initial;" v-has="'fileMgr_handleAdd'" @click="handleAddFolder">新建文件夹</el-button>
                    </div>
                </el-header>
...
export default {
    ...
    methods:{
        ...
        getList(){
            ...
            this.$http({
                method: 'GET',
                url: this.api+'/api/Common/GetFileManagePageBySearch?pageSize=' + self.page.pageSize + '&pageIndex=' + self.page.pageNo+'&id='+self.BusinessType+'&startDate='+self.queryparam.startDate+'&endDate='+self.queryparam.endDate
            }).then(res => {
                if(res.status==200){
        ...
        handleRowdblclick(row, column, event) {//双击行
            ...
            if(row.fileCType==0){//文件夹
                ...
                this.$http({
                    method: 'GET',
                    url: this.api+'/api/Common/GetFileManagePageBySearch?pageSize=10000&pageIndex=1&id='+row.id
                }).then(res => {
                    if(res.status==200){
                        self.list=res.data.data;
                        self.page.total = res.data.total;
        ...
        commonDownLoad(row){
            ...
            this.$http({
                method: 'GET',
                url: self.api+'/api/DownLoad/commonGetDownLoadPath?partialPath='+row.fileUrl
            }).then(res => {
                if(res.data.code==200){
                    location.href = self.api + '/api/DownLoad/commonDownLoad?fileName='+row.fileName+'&path='+res.data.data; 
                    self.$http({
                        method: 'GET',
        ...
        submitForm(){
            ...
            this.$http({
                method: 'GET',
                url: self.api+'/api/Upload/UploadFolder?BusinessType='+self.BusinessType+'&FileName='+self.FileName
            }).then(res => {
                if(res.data.code==200){
                    self.isFolder=false;
                    self.getList();  
