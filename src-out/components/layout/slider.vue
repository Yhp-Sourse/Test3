<template>
  <div id="v_slider">
    ...
    <el-menu
      :default-openeds="['200', '3', '4', '5', '6', '2020082116471091274']"
      router
    ...
    >
      ...
      >
        <template v-slot:title>
          ...
          <template v-slot:title>
            <span>{{ first.menu_name }}</span>
          </template>
        </template>
        <template v-if="isSubmenu">
          <el-submenu
            v-for="second in first.children"
            :index="second.menu_url"
            :key="second.menu_id"
          >
            <template v-slot:title>
              <span>{{ second.menu_name }}</span>
            </template>
            <el-menu-item
              v-for="third in second.children"
              :index="third.menu_url"
              :key="third.menu_id"
...
export default {
  ...
  methods: {
    InitSecondMenu() {
      ...
      if (
        !(
          typeof menuData_cache == 'string' &&
          JSON.parse(menuData_cache) != null
        )
      ) {
        this.$http({
          method: 'GET',
          ...
          .then((res) => {
            if (res.status == 200) {
              ...
              var first = Enumerable.from(data)
                .where('p=>p.menu_level==1')
                .toArray() //一级菜单
              var second = Enumerable.from(data)
                .where('p=>p.menu_level==2')
                .toArray() //二级菜单
              var third = Enumerable.from(data)
                .where('p=>p.menu_level==3')
                .toArray() //三级菜单
              third.forEach((second) => {
                second.children = Enumerable.from(data)
                  .where('p=>p.menu_pid==' + second.menu_id + '&p.menu_type==1')
                  .orderBy('p=>p.menu_order')
                  .toArray() //将三级菜单挂载在二级菜单下
              })
              second.forEach((first) => {
                first.children = Enumerable.from(data)
                  .where('p=>p.menu_pid==' + first.menu_id + '&p.menu_type==1')
                  .orderBy('p=>p.menu_order')
                  .toArray() //将二级菜单挂载在一级菜单下
              })
              first.forEach((first) => {
                first.children = Enumerable.from(data)
                  .where('p=>p.menu_pid==' + first.menu_id + '&p.menu_type==1')
                  .orderBy('p=>p.menu_order')
                  .toArray() //将一级菜单挂载在this.menuData下
              })
              self.treeMenu = first
        ...
        third.forEach((second) => {
          second.children = Enumerable.from(data)
            .where('p=>p.menu_pid==' + second.menu_id)
            .orderBy('p=>p.menu_order')
            .toArray() //将三级菜单挂载在二级菜单下
        })
        second.forEach((first) => {
          first.children = Enumerable.from(data)
            .where('p=>p.menu_pid==' + first.menu_id)
            .orderBy('p=>p.menu_order')
            .toArray() //将二级菜单挂载在一级菜单下
        })
        first.forEach((first) => {
        ...
        this.treeMenu = first
        this.menuData = Enumerable.from(first)
          .where('p=>p.menu_id==1')
          .toArray()
    ...
    showMenu(obj) {
      var self = this
      var Enumerable = require('linq')
      this.menuData = Enumerable.from(this.treeMenu)
        .where('p=>p.menu_id==' + obj.param[0])
    ...
    handleSelect(key, keyPath) {
      ...
      this.$router.options.routes.forEach((item) => {
        //遍历全局所有路由。并找到当前路由信息并跳转该路由。
        if (item.path == key) {
          $emit(this, 'parentFn', { param: item.name })
          return
        }
      })
    },
