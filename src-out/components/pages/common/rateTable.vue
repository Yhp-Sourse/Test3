  ...
  <div class="table">
        ...
        >
          <template v-slot="scope">
            <template v-if="!column.render">
              ...
              <template v-else>
                <span :title="scope.row[column.prop]">{{
        ...
        <template v-slot="scope">
          <div class="operate-group" style="overflow: auto">
            <template v-for="(btn, key) in operates.list">
              <span class="item" v-if="btn.show">
                <el-button
                  :type="btn.bgColortype"
                  :class="btn.className"
                  v-has="btn.hasbutton"
                  size="mini"
                  :disabled="btn.disabled"
                  @click.prevent="btn.method(key, scope.row)"
                  >{{ btn.label }}
                </el-button>
              </span>
            </template>
    ...
    >
      <i
        ...
        @click="foo"
      ></i>
      <el-checkbox-group v-model:value="colOptions">
...
export default {
  ...
  components: {
    //组件
    expandDom: function render(_props, _context) {
      const ctx = {
        ..._context,
        props: _props,
        data: _context.attr,
  ...
  mounted() {
    ...
    this.$nextTick(function () {
      self.options.tableHeight =
        window.innerHeight - self.$refs.mutipleTable.$el.offsetTop - 205
  ...
  methods: {
    ...
    handleSelectionChange(val) {
      ...
      $emit(this, 'handleSelectionChange', val)
    },
    handleCellClick(row, column, cell, event) {
      //	当某个单元格被点击时会触发该事件
      $emit(this, 'handleCellClick', { row, column, cell, event })
    },
    handleRowdblclick(row, column, event) {
      //当某一行被双击时会触发该事件	row, column, event

      $emit(this, 'handleRowdblclick', { row, column, event })
    },
    // 显示 表格操作弹窗
    showActionTableDialog() {
      $emit(this, 'handelAction')
    },
    handleSizeChange(val) {
      //每页显示多少条
      $emit(this, 'sizeChange', val)
    },
    handleCurrentChange(val) {
      //当前多少页
      $emit(this, 'currentPage', val)
    },
    clearSelection() {
      //清除表格chekbox
