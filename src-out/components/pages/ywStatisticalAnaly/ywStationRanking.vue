            ...
            <el-form :inline="true" class="demo-form-inline">
              ...
              <el-form-item class="btn">
                <el-button
                  type="primary"
                  icon="el-icon-search"
                  v-has="'stationRanking_handleSearch'"
                  @click="getList()"
                  >查询</el-button
                >
                <el-button
...
export default {
      ...
      columns: [
        ...
        { prop: 'ranks', label: '排名', align: 'center', isShow: true },
        {
          ...
          formatter: (row, column, cellValue) => {
            if (row.twoRateScore == null) {
              return '<a style="color:blue" > -- </a>'
            } else {
              return '<a style="color:blue" >' + row.twoRateScore + '</a>'
            }
          },
        },
        {
          prop: 'show_OperationScore',
          label: '运维得分',
          align: 'center',
          isShow: false,
          formatter: (row, column, cellValue) => {
            return '<a style="color:blue" >' + row.show_OperationScore + '</a>'
          },
        },
        {
          prop: 'show_CityScore',
          ...
          isShow: true,
          formatter: (row, column, cellValue) => {
            return '<a style="color:blue" >' + row.show_CityScore + '</a>'
          },
        },
        {
          prop: 'total',
          ...
          isShow: true,
          formatter: (row, column, cellValue) => {
            return '<a style="color:blue" >' + row.total + '</a>'
          },
        },
        {
          prop: 'provinceScore',
          ...
          isShow: false,
          formatter: (row, column, cellValue) => {
            if (row.provinceScore == null) {
              return '<a style="color:blue" > -- </a>'
            } else {
              return '<a style="color:blue" >' + row.provinceScore + '</a>'
            }
          },
        },
      ], // 需要展示的列
  ...
  methods: {
    ...
    getNowTime() {
      ...
      this.queryparam['SearchTime'] = defaultDate
    },
    ...
    getList(obj) {
      ...
      this.$http({
        ...
        url:
          this.api +
          '/api/OpsRanking/GetStationRank?pageSize=' +
          self.page.pageSize +
          '&pageIndex=' +
          self.page.pageNo +
          ' &station=' +
    ...
    download(obj) {
      ...
      this.$http({
        ...
        url:
          this.api +
          '/api/OpsRanking/GetStationRankDownLoad?pageSize=' +
          self.page.pageSize +
          '&pageIndex=' +
          self.page.pageNo +
          ' &station=' +
