<template>
  <div id="taskDisplay">
                  ...
                  <el-col :span="14">
                    <div>
                      <el-form-item label="任务类型：" prop="TaskType">
                        <rate-select
                          v-model:value="taskTypeRateSelect.model"
                  ...
                  <el-col :span="14">
                    <div>
                      <el-form-item label="紧急程度：" prop="Urgency">
                        <rate-select
                          v-model:value="urgencyRateSelect.model"
                  ...
                  <el-col :span="14">
                    <div>
                      <el-form-item label="运维单位：" prop="UnitName">
                        <rate-select
                          v-model:value="rateSelect.model"
                    ...
                    <div>
                      <el-form-item label="运维人员：" prop="DealerName">
                        <el-input
                          ...
                          autocomplete="off"
                          @click="
                            dialogVisibleYwPerson = !dialogVisibleYwPerson
                          "
                        ></el-input>
                      </el-form-item>
                    </div>
                  ...
                  <el-col :span="14">
                    <div>
                      <el-form-item label="处理方式：" prop="MaintainceWay">
                        <rate-select
                          v-model:value="maintainceWayRateSelect.model"
                  ...
                  <el-col :span="14">
                    <div>
                      <el-form-item label="任务类别：" porp="TaskCategoryId">
                        <rate-select
                          v-model:value="taskRateSelect.model"
                  ...
                  <el-col :span="14">
                    <div>
                      <el-form-item label="处理描述：" prop="ItemRemark">
                        <el-input
                          type="textarea"
                          v-model:value="ruleForm.ItemRemark"
                          autocomplete="off"
                        ></el-input>
                      </el-form-item>
                    </div>
                  ...
                  <el-col :span="14">
                    <div>
                      <el-form-item label="处理时间：" prop="ItemCreatedTime">
                        <el-date-picker
                          v-model:value="ruleForm.ItemCreatedTime"
                          type="datetime"
                          placeholder="选择时间"
                          style="width: 100%"
                        ></el-date-picker>
                  ...
                  <el-col :span="24">
                    <div class="tools">
                      <el-button
                        size="small"
                        class="el-button--iconButton"
                        v-show="actionStatus.fillForm"
                        v-has="'ywTask_handleWriteForm'"
                        icon="el-icon-plus"
                        @click="handleWriteForm('', '', 'add', '')"
                        >填写表单</el-button
                      >
                      <el-button
                        size="small"
                        class="el-button--iconButton"
                        v-show="actionStatus.judgeForm"
                        v-has="'ywTask_handleJudgeForm'"
                        icon="el-icon-plus"
                        @click="handleWriteForm"
                        >审核表单</el-button
                      >
                    </div>
                  </el-col>
                ...
                <el-col :span="24">
                  <div>
                    <el-form-item label="原因备注：" prop="YSJudgeRemark">
                      <el-input
                        type="textarea"
                        v-model:value="ruleForm.YSJudgeRemark"
                        autocomplete="off"
                        rows="4"
                      ></el-input>
                    </el-form-item>
                ...
                <el-col :span="12">
                  <div>
                    <el-form-item>
                      <el-button
                        type="primary"
                        @click="submitAllJudgeForm('ruleForm', '提交审核结果')"
                        >确定</el-button
                      >
                      <el-button
                        @click="
                          aduitFormDialogVisible = !aduitFormDialogVisible
                        "
                        >取消</el-button
                      >
                    </el-form-item>
    ...
    <el-dialog
      title="选择人员"
      v-model:visible="dialogVisibleYwPerson"
      width="35%"
    >
      <rateTransfer
        @choseSuccess="choseSuccess"
        :stationId="choosedSstationId"
      ></rateTransfer>
    </el-dialog>
  </div>
</template>
...
export default {
            ...
            method: (index, row) => {
              ...
              if (row.doCount >= 4) {
                this.$message.warning(
                  '此表单被拒绝4次，已被锁定，请联系管理员进行解锁!'
                )
                return false
              } else {
                this.handleWriteForm(
                  row.id,
                  row.reportCode,
  ...
  methods: {
    ...
    handleWriteForm(id, reportCode, secondType, status) {
      var self = this
      if (
        self.firstButtonType == 'Detail' &&
        self.secondButtonType != 'detail'
      ) {
        self.$message({
          message: '查看状态下按钮不可用!',
          type: 'warning',
      ...
      var obj = {
        taskNo: this.ruleForm.TaskNo,
        id: id,
        secondButtonType: this.secondButtonType,
        rptCode: reportCode,
        rptType: this.ruleForm.TaskCategoryId,
        chooseTaskCategory: this.chooseTaskCategory,
        chooseStatus: this.chooseStatus,
        chooseStationIds: this.chooseStationIds,
        firstButtonType: self.firstButtonType,
        status: status,
        chooseStartTime: this.chooseStartTime,
        chooseEndTime: this.chooseEndTime,
      }
      $emit(this, 'jump', {
        param: '运维表单',
        path: '/MaintainceRpt?obj=' + JSON.stringify(obj),
        isjump: true,
      })
      //关闭当前tab页面
      $emit(this, 'closeCurrentPage', '任务编辑') //调用closeCurrentPage方法注意；两点1：第二个参数必须是string，2：长度必须大于等于4
    },
    ...
    handleReturn() {
      ...
      var obj = {
        taskNo: this.ruleForm.TaskNo,
        taskCategoryId: this.chooseTaskCategory,
        chooseStartTime: this.chooseStartTime,
        chooseEndTime: this.chooseEndTime,
        sStations: this.chooseStationIds,
        taskStatus: this.chooseStatus,
        flag: 2,
      }
      $emit(this, 'jump', {
        param: '运维任务',
        path: '/index/ywTask?obj=' + JSON.stringify(obj),
        isjump: true,
      })
    },
          ...
          this.$refs[formName].validate((valid) => {
            if (valid) {
              ...
              this.$http({
                ...
                url:
                  self.api +
                  '/api/Yw_Task/HaveRight?taskNo=' +
                  self.ruleForm.TaskNo +
                  '&planDealTime=' +
                  self.ruleForm.PlanDealTime +
                  '&taskCategoryId=' +
        ...
        .then((res) => {
          if (res.status == 200) {
            if (res.data.data != null) {
              ...
              self.maintainceWayRateSelect.model =
                res.data.data.show_MaintainceWay
              ...
              if (res.data.data.taskRCItem != null) {
                ...
                self.ruleForm.ItemCreatedTime =
                  res.data.data.taskRCItem.itemCreatedTime
              }
              ...
              if (self.ruleForm.Status == 4 || self.ruleForm.Status == 6) {
                //待审核，审核完的按钮无效
                //表示需要进行表单添加
